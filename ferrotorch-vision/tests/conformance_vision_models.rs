...
// ---------------------------------------------------------------------------
// #983 — Phase 1A: ResNet50 value-parity vs torchvision reference
//
// The pipeline mechanically replicated for the rest of Sprint 1B / 1C:
...
//      regenerate_vision_fixtures.py --models resnet50` running torchvision
...
//      structurally incomplete fixture is rejected), then strips buffer
//      keys before calling Module::load_state_dict in strict mode.
...
//
// Failure modes this test is built to catch (project-specific):
//   - shape-only-dressed-as-value: we compare every element, not shape.
//   - tautological reference: the reference is produced by torchvision in
//     Python, NEVER by running ferrotorch and recording the output.
//   - partial weight load passes: the loader errors when any ferrotorch
//     parameter is missing from the file or any extra key is present.
//   - silent CPU fallback: the test stays on CPU end-to-end; when the
//     model gains a `.to(Device::Cpu)` API or runs forward on a non-Cpu
//     device, this assertion will need an explicit device check.
// ---------------------------------------------------------------------------

mod resnet50_value_parity_pipeline {
    ...
    use ferrotorch_vision::models::resnet50;
    use serde::Deserialize;

    // ── Descriptor schema ────────────────────────────────────────────────

    #[derive(Debug, Deserialize)]
    struct ValueParityDescriptor {
        id: String,
        weights_path: String,
        input_path: String,
    ...
    /// Outcome of the strict torchvision → ferrotorch state-dict adoption.
    /// On success the model has every parameter overwritten from `state`;
    /// on failure it is left in whatever state the operation reached.
    ///
    ...
    pub(super) fn load_torchvision_state_into_resnet50(
        model: &mut ferrotorch_vision::models::ResNet<f32>,
        state: &StateDict<f32>,
        ...
        for key in state.keys() {
            ...
            if !ft_param_set.contains(s) && !buffer_set.contains(s) {
                return Err(FerrotorchError::InvalidArgument {
                    message: format!(
                        "ResNet50 value-parity loader: torchvision key \"{key}\" \
        ...
        for buf_name in expected_buffer_keys {
            if !state.contains_key(buf_name) {
                return Err(FerrotorchError::InvalidArgument {
                    message: format!(
                        "ResNet50 value-parity loader: expected BN buffer key \
        ...
        for ft_name in &ft_param_names {
            if !state.contains_key(ft_name) {
                return Err(FerrotorchError::InvalidArgument {
                    message: format!(
                        "ResNet50 value-parity loader: ferrotorch parameter \
        ...
        for (name, param) in ft_param_names.iter().zip(params_mut) {
            ...
            if param.shape() != tensor.shape() {
                return Err(FerrotorchError::ShapeMismatch {
                    message: format!(
                        "ResNet50 value-parity loader: shape mismatch for \
    ...
    fn maybe_load_fixture() -> Option<LoadedFixture> {
        let descriptor = load_descriptor("resnet50_value_parity")?;
        ...
        let state: StateDict<f32> =
            load_safetensors(&weights).expect("load_safetensors(resnet50 weights)");
    ...
    fn skip_unless_artefacts_present() -> LoadedFixture {
        match maybe_load_fixture() {
            Some(t) => t,
            None => panic!(
                "resnet50 value-parity artefacts not found. Run: \
    ...
    fn resnet50_value_parity() {
        let LoadedFixture {
            ...
            input: input_data,
            expected: expected_data,
        } = skip_unless_artefacts_present();

        // Sanity-check the descriptor against the binary blob lengths so a
        // truncated regenerate is rejected immediately.
        let in_numel: usize = descriptor.input_shape.iter().product();
        let out_numel: usize = descriptor.output_shape.iter().product();
        assert_eq!(
            input_data.len(),
            in_numel,
            "input.bin length {} != product(input_shape)={}",
            input_data.len(),
            in_numel
        );
        assert_eq!(
            expected_data.len(),
            out_numel,
            "expected_output.bin length {} != product(output_shape)={}",
            expected_data.len(),
            out_numel
        );

        // Confirm the safetensors file contains exactly the parameter keys
        // and BN buffer keys the descriptor claims, and nothing else (the
        // descriptor pins the contract, the loader enforces it).
        ...
        assert!(
            ...
            "safetensors key mismatch with descriptor — extra={:?} missing={:?}",
            extra,
            missing
        );
        // The descriptor records integer-typed BN buffers (num_batches_tracked)
        // that we deliberately exclude from the safetensors payload.
        for skipped in &descriptor.skipped_int_buffer_keys {
            assert!(
                !on_disk_keys.contains(skipped.as_str()),
                "integer buffer \"{skipped}\" must NOT be present in fixture"
            );
        }

        // Build the model and explicitly target CPU. ResNet50::new builds on
        // CPU by default, but we re-assert here so a future implicit-GPU
        // change cannot make this test silently CPU-pull.
        let mut model = resnet50::<f32>(1000).expect("resnet50 construction");
        for p in model.parameters() {
            assert_eq!(
                p.tensor().device(),
                Device::Cpu,
                "Phase 1A: parameter must live on CPU"
            );
        }

        let buffer_keys = buffer_keys_from_descriptor(&descriptor);
        load_torchvision_state_into_resnet50(&mut model, &state, &buffer_keys)
            .expect("loader succeeds on a well-formed fixture");

        // Run forward in eval mode — both reference and ferrotorch sides use
        // default BN running stats (mean=0, var=1), so eval-mode equivalence
        // holds without loading buffers in this phase.
        model.eval();

        let input_tensor: Tensor<f32> = Tensor::from_storage(
            TensorStorage::cpu(input_data),
        ...
        let output = ferrotorch_core::no_grad(|| model.forward(&input_tensor).expect("forward"));
        assert_eq!(output.device(), Device::Cpu, "forward must stay on CPU");
        ...
        // Element-wise comparison. Track max-abs and max-rel for the report
        // so audit can sanity-check the chosen tolerance.
        ...
        for (i, (&a, &e)) in actual.iter().zip(expected_data.iter()).enumerate() {
            assert!(
                ...
                "non-finite value at {i}: actual={a} expected={e}"
            );
            let abs = (a - e).abs();
        ...
        eprintln!(
            "resnet50_value_parity: max_abs_err={max_abs:.3e} (idx {worst_idx}) \
             max_rel_err={max_rel:.3e} \
             abs_tol={:.3e} rel_tol={:.3e} numel={}",
        ...
        assert!(
            max_abs <= descriptor.abs_tolerance || max_rel <= descriptor.rel_tolerance,
            "ResNet50 value parity: max_abs={max_abs:.3e} (tol {:.3e}), \
             max_rel={max_rel:.3e} (tol {:.3e}) — neither tolerance satisfied. \
             Worst index {worst_idx}: actual={} expected={}",
            descriptor.abs_tolerance,
            descriptor.rel_tolerance,
            actual[worst_idx],
            expected_data[worst_idx],
        );
    }

    // ── Loader honest-fail probes ────────────────────────────────────────
    //
    ...
    // human-readable message. The model under test is freshly constructed
    // before each probe so a partial mutation never leaks into the next.

    fn fresh_model() -> ferrotorch_vision::models::ResNet<f32> {
        resnet50::<f32>(1000).expect("resnet50 construction")
    }

    #[test]
    fn loader_rejects_unmapped_torchvision_key() {
        let Some(mut fix) = maybe_load_fixture() else {
            panic!("artefacts missing; run regenerate_vision_fixtures.py --models resnet50");
        };
        // Inject a key that is neither a ferrotorch parameter nor a declared
        // BN buffer.
        fix.state.insert(
        ...
        let err = load_torchvision_state_into_resnet50(&mut model, &fix.state, &buffer_keys)
            .expect_err("must reject unmapped key");
        let msg = format!("{err}");
        assert!(
    ...
    #[test]
    fn loader_rejects_missing_ferrotorch_param() {
        let Some(mut fix) = maybe_load_fixture() else {
            panic!("artefacts missing; run regenerate_vision_fixtures.py --models resnet50");
        };
        // Drop a key that ferrotorch's resnet50 expects (fc.bias). The
        // file-side enumeration already excludes `fc.bias`, so the loader's
        // "ferrotorch parameter has no source" branch triggers.
        let _ = fix
            .state
            .remove("fc.bias")
            .expect("fc.bias present in fixture");
        let mut model = fresh_model();
        let buffer_keys = buffer_keys_from_descriptor(&fix.descriptor);
        let err = load_torchvision_state_into_resnet50(&mut model, &fix.state, &buffer_keys)
            .expect_err("must reject missing ferrotorch param");
        let msg = format!("{err}");
        assert!(
            msg.contains("fc.bias") && msg.contains("no source"),
            "expected message to name the missing ferrotorch param, got: {msg}"
        );
    }

    #[test]
    fn loader_rejects_shape_mismatch() {
        let Some(mut fix) = maybe_load_fixture() else {
            panic!("artefacts missing; run regenerate_vision_fixtures.py --models resnet50");
        };
        // Replace fc.weight with a wrong-shape tensor.
        fix.state.insert(
            "fc.weight".to_string(),
        ...
        let msg = format!("{err}");
        assert!(
            msg.contains("fc.weight") && msg.contains("shape mismatch"),
            "expected shape-mismatch message naming fc.weight, got: {msg}"
        );
    }

    #[test]
    fn loader_rejects_missing_bn_buffer() {
        let Some(mut fix) = maybe_load_fixture() else {
            panic!("artefacts missing; run regenerate_vision_fixtures.py --models resnet50");
        };
        let target = "bn1.running_mean";
        assert!(
        ...
        let buffer_keys = buffer_keys_from_descriptor(&fix.descriptor);
        let err = load_torchvision_state_into_resnet50(&mut model, &fix.state, &buffer_keys)
            .expect_err("must reject missing BN buffer");
        let msg = format!("{err}");
        assert!(
