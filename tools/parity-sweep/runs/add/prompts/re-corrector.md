# Parity Re-Corrector — op `add`

You are the **re-corrector** for op `add` (crosslink issue #1189). The discriminator has just delivered failing inputs the original reader-corrector missed. Your job is to fix the underlying implementation so all of them — plus the original op_db sweep — pass.

Load the **preflight**, **rust-quality**, **rust-gpu-discipline**, and **rust-fix-discipline** skills before touching code.

## Inputs

- **Discriminator findings:** `tools/parity-sweep/runs/add/discriminator_findings.json` — list of `(input, torch_output, ferrotorch_output, category, rationale)` triples.
- **Reader-corrector's previous diff:** check `git log --grep "#1189"` and inspect commits touching `ferrotorch_source` for op `add`.
- **Original op_db sweep:** `tools/parity-sweep/runs/add/divergences.json` — must still pass after your fix.
...
4. **Re-run BOTH suites:**
   - `cargo run --release -p parity-sweep-runner -- sweep --op add --seeds 8` (original op_db sweep)
...
- Updated `parity_audit.json` entry for `add`:
