...
impl<T: Float> Tensor<T> {
    ...
    pub fn clamp_tensor_t(
        &self,
        min: Option<&Tensor<T>>,
        max: Option<&Tensor<T>>,
    ) -> FerrotorchResult<Tensor<T>> {
        crate::grad_fns::transcendental::clamp_tensor(self, min, max)
    }

    pub fn clamp_min_tensor_t(&self, min: &Tensor<T>) -> FerrotorchResult<Tensor<T>> {
        crate::grad_fns::transcendental::clamp_min_tensor(self, min)
    }

    pub fn clamp_max_tensor_t(&self, max: &Tensor<T>) -> FerrotorchResult<Tensor<T>> {
        crate::grad_fns::transcendental::clamp_max_tensor(self, max)
    }
    ...
    pub fn clip_tensor_t(
        &self,
        min: Option<&Tensor<T>>,
        max: Option<&Tensor<T>>,
    ) -> FerrotorchResult<Tensor<T>> {
        crate::grad_fns::transcendental::clip_tensor(self, min, max)
    }
    ...
    pub fn transpose(&self, dim0: usize, dim1: usize) -> FerrotorchResult<Tensor<T>> {
        let ndim = self.ndim();
        // torch accepts `transpose(0, 0)` on a 0-D tensor: `maybe_wrap_dim`
        // wraps scalar dims over the range `[-1, 0]`, of which 0 is the only
        // `usize`-expressible member (CORE-153 / #1847). Any other dim still
        // errors below, matching torch's IndexError.
        if ndim == 0 && dim0 == 0 && dim1 == 0 {
...
pub fn contiguous_t<T: Float>(input: &Tensor<T>) -> FerrotorchResult<Tensor<T>> {
    ...
    // A tensor whose strides look C-contiguous is safe to cheap-clone only when
    // it addresses the whole underlying storage from offset zero. PyTorch's
    // contiguity bit is shape/stride based, so `narrow(0, 0, 1).squeeze()` can
    // be contiguous with offset 0 while its storage still contains the entire
    // base tensor. CUDA consumers receive only a raw buffer handle, so they
    // would reduce/copy past the logical view unless we materialise a packed
    // buffer here (#1657/#1961).
...
pub fn chunk_t<T: Float>(
    ...
    // ATen special case (CORE-152 / #1846): chunking an empty dimension
    // returns `chunks` zero-sized tensors — `chunk` never returns an empty
    // list. Mirrors `aten/src/ATen/native/TensorShape.cpp` `chunk`:
    // `split_size == 0 && dim_size == 0` →
    // `split_with_sizes(vec![0; chunks], dim)`. Live torch 2.11:
    // `torch.empty(0, 3).chunk(3, 0)` yields three `(0, 3)` tensors.
    if dim_size == 0 {
        return split_t(input, &vec![0; chunks], dim);
    }
