...
pub fn unique_consecutive<T: Float>(
    ...
    // GPU fast path (#1545): for CUDA-resident float dtypes the run compaction
    // runs entirely on-device (run-flag → prefix-sum → scatter) via
...
pub fn meshgrid_indexing<T: Float>(
    ...
    if all_cuda && (is_f32::<T>() || is_f64::<T>() || is_f16::<T>() || is_bf16::<T>()) {
        let backend =
            crate::gpu_dispatch::gpu_backend().ok_or(FerrotorchError::DeviceUnavailable)?;
