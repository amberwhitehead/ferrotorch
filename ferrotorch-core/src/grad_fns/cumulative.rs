...
//! | REQ-3 (cummax) | SHIPPED | `cummax` at `cumulative.rs:671` + `CummaxBackward` at `:460` via private helper `cummaxmin_backward_impl` (scatter_add VJP per `derivatives.yaml:533-535`); consumer `Tensor::cummax_t` in `methods.rs`; parity `[cummax] 24/24` (grep=1) |
//! | REQ-4 (cummin) | SHIPPED | `cummin` at `cumulative.rs:712` + `CumminBackward` at `:501` shares the private `cummaxmin_backward_impl` helper; consumer `Tensor::cummin_t` in `methods.rs`; parity `[cummin] 24/24` (grep=1) |
...
//! | REQ-6 (dim normalization) | SHIPPED | `normalize_axis(dim as isize, ndim)` calls at `cumulative.rs:109, :404, :675, :716, :1004` mirroring `maybe_wrap_dim` at `ReduceOps.cpp:506, :622, :851, :890`; consumers are the five `pub fn` bodies themselves (reached through the `methods.rs` `*_t` surfaces) |
