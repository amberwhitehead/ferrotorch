...
fn clamp_with_forward_and_backward_bounds<T: Float>(
    ...
    let output = unary_map_named(input, "clamp", |x| {
        clamp_scalar_pair(x, forward_min, forward_max)
    })?;
    if needs_grad_unary(input) {
        let (storage, shape) = output.into_storage_and_shape()?;
        Tensor::from_operation(
...
fn attach_clamp_tensor_backward<T: Float>(
    ...
    Tensor::from_operation(
        ...
        Arc::new(ClampTensorBackward {
            input: input.saved_for_backward()?,
            min: min.map(Tensor::saved_for_backward).transpose()?,
            max: max.map(Tensor::saved_for_backward).transpose()?,
        }),
    )
}
...
/// enabled. Mirrors the CPU-only named-unary pattern used by ops without a
/// GPU-layer kernel.
#[inline]
fn finish_unary<T: Float, G: GradFn<T> + 'static>(
    output: Tensor<T>,
    input: &Tensor<T>,
    make_grad: impl FnOnce() -> FerrotorchResult<G>,
) -> FerrotorchResult<Tensor<T>> {
    if needs_grad_unary(input) {
        let (storage, shape) = output.into_storage_and_shape()?;
...
fn finish_unary_saving_output<T: Float, G: GradFn<T> + 'static>(
    output: Tensor<T>,
    input: &Tensor<T>,
    make_grad: impl FnOnce(Tensor<T>) -> FerrotorchResult<G>,
) -> FerrotorchResult<Tensor<T>> {
    if needs_grad_unary(input) {
        let (storage, shape) = output.into_storage_and_shape()?;
        Tensor::from_operation_saving_output(storage, shape, |saved_output| {
            Ok(Arc::new(make_grad(saved_output)?))
        })
    } else {
        Ok(output)
    }
...
impl<T: Float> GradFn<T> for FracBackward<T> {
    fn backward(&self, grad_output: &Tensor<T>) -> FerrotorchResult<Vec<Option<Tensor<T>>>> {
        let da = if self.input.requires_grad() {
            let one = <T as num_traits::One>::one();
            // CUDA path (#1743 / CORE-049): closure-derived VJP has no
            // resident kernel yet, so surface the named unsupported op.
            if self.input.is_cuda() || grad_output.is_cuda() {
                return Ok(vec![Some(cuda_unary_vjp(
                    grad_output,
                    &self.input,
...
pub fn sinc<T: Float>(input: &Tensor<T>) -> FerrotorchResult<Tensor<T>> {
    ...
    let zero = <T as num_traits::Zero>::zero();
    let one = <T as num_traits::One>::one();
    let output = unary_map_named(input, "sinc", |x| {
        if x == zero {
            one
        } else {
            let px = pi * x;
