...
//! | REQ-4 (comparison / logical predicate surface) | SPLIT-OWNERSHIP | This module owns differentiable `where` only. Float and integer `eq`/`ne`/`lt`/`le`/`gt`/`ge` plus bool `logical_and`/`logical_or`/`logical_xor`/`logical_not` live in `bool_tensor.rs` and dispatch resident CUDA kernels; CUDA broadcasted i32/i64 comparisons route through `GpuBackend::compare_broadcast` rather than a CPU value round trip. Other value-returning extrema / predicate APIs are owned by their dedicated modules. |
