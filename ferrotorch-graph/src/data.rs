...
//! | REQ-3 | SHIPPED | impl: `Graph::num_nodes` / `Graph::num_features` accessors in `data.rs`; non-test consumer: `ferrotorch-graph/examples/gcn_inference_dump.rs` builds `Graph::new(x, edge_index, zero_labels)?` and reads `graph.num_nodes()` / `graph.num_features()` for the diagnostic log line + the debug_assert that pins the COO/Tensor coherence invariant. Closes #1481. |
//! | REQ-4 | SHIPPED | impl: `Graph::edge_src(e)` / `Graph::edge_dst(e)` `#[inline]` accessors in `data.rs`; non-test consumer: `ferrotorch-graph/examples/gcn_inference_dump.rs` invokes `graph.edge_src(0)` / `graph.edge_dst(0)` on the first edge for the diagnostic log line; the broader forward path consumes `graph.edge_index` (the validated buffer) directly through `net.forward(&graph.x, &graph.edge_index)`. Closes #1481. |
