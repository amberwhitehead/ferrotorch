...
fn audit_1375_supported_pair_count() {
    ...
    // analog, #1375) exposes 70 pairs after the #1562 both-types-exist KL
    // additions and the #1374 Binomial sub-part (Binomial-Binomial +
    // Poisson-Binomial); the precise count is also drift-checked by
    // `kl::tests::kl_doc_table_matches_dispatcher` and
    // `divergence_wave_m_audit::audit_1374_supported_pair_count_is_41`.
    assert_eq!(kl_supported_pair_count(), 70);
}
