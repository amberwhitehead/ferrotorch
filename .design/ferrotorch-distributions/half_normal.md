...
| REQ-10 | SHIPPED | impl: `has_rsample`(=true) / `batch_shape` / `support`(`NonNegative` per `half_normal.py:35`) / `arg_constraints`(`{scale: Positive}` per `half_normal.py:33`) / `expand` / `cdf`(via `erf`) / `icdf`(via `erfinv`) overrides at the tail of `impl Distribution for HalfNormal` in `half_normal.rs` mirroring `torch/distributions/half_normal.py:33-36`; non-test consumer: trait dispatch through `pub use HalfNormal` re-export at `lib.rs:105`; `test_half_normal_cdf_at_zero_is_zero` / `test_half_normal_cdf_icdf_roundtrip` / `test_half_normal_surface_overrides` / `test_half_normal_expand` pin the overrides. Closes #1421. |
