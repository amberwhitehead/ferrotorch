...
| REQ-12 | SHIPPED | impl: `has_rsample`(=true) / `batch_shape` / `support`(`Real` per `gumbel.py:35`) / `arg_constraints`(`{loc: Real, scale: Positive}` per `gumbel.py:33`) / `expand` overrides at the tail of `impl Distribution for Gumbel` in `gumbel.rs`; non-test consumer: trait dispatch through `pub use Gumbel` re-export at `lib.rs:104`; `test_gumbel_surface_overrides` and `test_gumbel_expand` pin the overrides. Closes #1419 — `validate_args` remains under #1376. |
