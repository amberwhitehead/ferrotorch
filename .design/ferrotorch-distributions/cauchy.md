...
| REQ-11 | SHIPPED | impl: `has_rsample` / `batch_shape` / `support` / `arg_constraints` / `expand` overrides at the tail of `impl Distribution for Cauchy` in `cauchy.rs` mirroring `torch/distributions/cauchy.py:34-36`; non-test consumer: trait dispatch through `pub use cauchy::Cauchy` re-export at `lib.rs`; `test_cauchy_surface_overrides` and `test_cauchy_expand` pin the override values. Closes #1400 partial — `validate_args` and scalar-broadcast ctor remain under #1376. |
