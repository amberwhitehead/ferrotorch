...
| REQ-9 | PARTIAL | impl: `has_rsample` / `has_enumerate_support` / `support` (BooleanConstraint) / `arg_constraints` (probs:UnitInterval) / `event_shape` / `enumerate_support` (yields `[0,1]` mirroring `bernoulli.py:138-145`) / `expand` (broadcasts `probs`) trait overrides at the tail of `impl Distribution<T> for Bernoulli<T>` in `bernoulli.rs`; non-test consumer: `pub use bernoulli::Bernoulli` at `lib.rs`; `tests/divergence_distribution_trait_surface.rs::bernoulli_*` pins every override. Closes #1406 — STILL NOT-STARTED: `logits` constructor + `validate_args` (orthogonal trackers). |
