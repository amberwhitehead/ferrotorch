...
| REQ-14 | SHIPPED | impl: `has_rsample` / `support` (a `HalfOpenInterval<f64>` materialised from `low`/`high` first-element bounds, mirroring `uniform.py:80-83`) / `arg_constraints` (low:Real, high:Real — inter-parameter `low<high` left to the constructor's R-DEV-4 shape-check path) / `event_shape` / `expand` (broadcasts both bounds) trait overrides at the tail of `impl Distribution<T> for Uniform<T>` in `uniform.rs`; non-test consumer: `pub use uniform::Uniform` at `lib.rs`; `tests/divergence_distribution_trait_surface.rs::uniform_*` pins every override. Closes #1430. |
