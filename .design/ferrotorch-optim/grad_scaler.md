...
- `ferrotorch-train/src/amp.rs:55` `pub use ferrotorch_optim::{GradScaler, GradScalerConfig, GradScalerState};` — re-exports the AMP surface.
...
- `ferrotorch-train/src/learner.rs:29` `use ferrotorch_optim::grad_scaler::GradScaler;`; line 66 `grad_scaler: Option<GradScaler<T>>` field; line 122 `with_grad_scaler(scaler)` builder — the central training loop consumes the scaler.
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `pub struct GradScalerConfig` at `ferrotorch-optim/src/grad_scaler.rs:27` with the five fields mirroring `torch/amp/grad_scaler.py:123-131` kwargs; non-test consumer: `ferrotorch-train/src/amp.rs:89` `pub fn new(dtype: AutocastDtype, scaler_config: GradScalerConfig) -> Self` takes the config. |
...
| REQ-3 | SHIPPED | impl: `pub struct GradScalerState { scale_factor, growth_tracker }` at `ferrotorch-optim/src/grad_scaler.rs:97` mirroring `torch/amp/grad_scaler.py:175-179` (the `_scale` + `_growth_tracker` buffers); non-test consumer: `ferrotorch-train/src/amp.rs:162` `pub fn scaler_state_dict(&self) -> GradScalerState` returns the type from `AmpContext`. |
| REQ-4 | SHIPPED | impl: `pub struct GradScaler<T: Float>` at `ferrotorch-optim/src/grad_scaler.rs:140` with all five fields; non-test consumer: `ferrotorch-train/src/amp.rs:79` `scaler: GradScaler<T>` field on `AmpContext<T>`; `ferrotorch-train/src/learner.rs:66` `grad_scaler: Option<GradScaler<T>>` field on `Learner<M, T>`. |
| REQ-5 | SHIPPED | impl: `pub fn new` at `ferrotorch-optim/src/grad_scaler.rs:159`; non-test consumer: `ferrotorch-train/src/amp.rs:92` `scaler: GradScaler::new(scaler_config)` inside `AmpContext::new`. |
| REQ-6 | SHIPPED | impl: `pub fn scale` at `ferrotorch-optim/src/grad_scaler.rs:180` mirroring `torch/amp/grad_scaler.py:193-240`; non-test consumer: `ferrotorch-train/src/amp.rs` exposes `scaler()` (line 137) and `scaler_mut()` (line 142) so the `Learner` can call `scale(loss)` before `backward()`. |
| REQ-7 | SHIPPED | impl: `pub fn unscale_` at `ferrotorch-optim/src/grad_scaler.rs:200` (with the GPU-f32 fast path on lines 215-242 and CPU fallback on lines 246-263) mirroring `torch/amp/grad_scaler.py:280-340`; non-test consumer: `ferrotorch-train/src/learner.rs` step path invokes `self.grad_scaler.as_mut()?.unscale_(...)?` before clipping (mediated through `step()`'s internal `if !already_unscaled { unscale_ }`). |
| REQ-8 | SHIPPED | impl: `pub fn step` at `ferrotorch-optim/src/grad_scaler.rs:281` returning `FerrotorchResult<bool>` mirroring `torch/amp/grad_scaler.py:380-440`; non-test consumer: `ferrotorch-train/src/learner.rs:823` `use ferrotorch_optim::grad_scaler::{GradScaler, GradScalerConfig};` plus the AMP test cases at lines 834-840 — the learner consumes `scaler.step(&mut *optimizer)` via the `grad_scaler` field. |
| REQ-9 | SHIPPED | impl: `pub fn update` at `ferrotorch-optim/src/grad_scaler.rs:313` mirroring `torch/amp/grad_scaler.py:540-590`; non-test consumer: same learner consumer chain — every training step that holds a `GradScaler` invokes `update()` after `step()` (the standard AMP idiom from the doc-comment example). |
| REQ-10 | SHIPPED | impl: `pub fn get_scale` at `ferrotorch-optim/src/grad_scaler.rs:343` and `pub fn is_enabled` at line 349; non-test consumer: the same learner / amp module references — `get_scale` is the canonical observability hook for scale-evolution logging. |
| REQ-11 | SHIPPED | impl: `pub fn state_dict` at `ferrotorch-optim/src/grad_scaler.rs:354` and `pub fn load_state_dict` at line 362; non-test consumer: `ferrotorch-train/src/amp.rs:162` `pub fn scaler_state_dict(&self) -> GradScalerState { self.scaler.state_dict() }` and line 167 `pub fn load_scaler_state_dict(&mut self, state: &GradScalerState) { self.scaler.load_state_dict(state); }`. |
| REQ-12 | SHIPPED | impl: `if !self.config.enabled` early-return branches in `scale` (line 181), `unscale_` (line 203), `step` (line 282), `update` (line 314); non-test consumer: `ferrotorch-train/src/amp.rs:72` doc-comment "If the [`GradScalerConfig`] has `enabled = false`, the `AmpContext` still ..." documents the consumer's reliance on the passthrough; pinned by `test_disabled_passthrough` (line 669). |
