...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `pub fn diff_sgd_step` at `diff_sgd_step in ferrotorch-optim/src/differentiable.rs` using `mul`/`sub` from `ferrotorch_core::grad_fns::arithmetic`; non-test consumer: `pub use differentiable::diff_sgd_step` at `ferrotorch-optim/src/lib.rs` — boundary-method public API per goal.md S5. |
...
| REQ-5 | SHIPPED | impl: `pub fn diff_sgd_momentum_step` at `diff_sgd_momentum_step in ferrotorch-optim/src/differentiable.rs` with the velocity recurrence implemented on lines 159-183; non-test consumer: `pub use differentiable::diff_sgd_momentum_step` at `ferrotorch-optim/src/lib.rs`. |
...
| REQ-7 | SHIPPED | impl: `pub type DiffSgdMomentumOutput<T> = (Vec<Tensor<T>>, Vec<Tensor<T>>)` at `diff_sgd_momentum_step in ferrotorch-optim/src/differentiable.rs`; non-test consumer: appears in the return type of the immediately-following `diff_sgd_momentum_step` (line 116), which is re-exported via `lib.rs`. |
| REQ-8 | SHIPPED | impl: every arithmetic call routes through `ferrotorch_core::grad_fns::arithmetic::{add, mul, sub}` (imports at `test_diff_sgd_step_autograd_edge_to_param in ferrotorch-optim/src/differentiable.rs`); non-test consumer: pinned by `test_diff_sgd_step_autograd_edge_to_param` and `test_diff_sgd_momentum_maintains_autograd_chain` in the in-file tests, plus the conformance-fixture integration tests in `ferrotorch-optim/tests/conformance_optim_advanced.rs` (lines 1264, 1321) that match against PyTorch `differentiable=True` reference. |
