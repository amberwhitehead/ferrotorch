...
| REQ-5 | SHIPPED | impl: Two-phase vs three-phase branching in the constructor (`scheduler/one_cycle_lr.rs`) mirrors `torch/optim/lr_scheduler.py:2454-2520`; non-test consumer: any `Learner` constructed with `OneCycleLR::new(..., true)` exercises the three-phase path via the `sched.step(...)` invocation at `new in ferrotorch-train/src/learner.rs`. Tests `test_one_cycle_three_phase` pins the three-phase end-of-schedule LR. |
