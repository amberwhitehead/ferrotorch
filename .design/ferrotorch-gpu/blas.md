...
| REQ-8 | SHIPPED | impl: `pub fn gpu_matmul_f32_into in blas.rs` and `pub fn gpu_bmm_f32_into in blas.rs`. Non-test consumer: the same `backend_impl.rs` dispatch table consumes these on the zero-host-allocation path (the `_into` family is reached when the consumer needs the matmul result to stay on the GPU; see the `_into` calls in `backend_impl.rs` around the f32 matmul/bmm dispatch). |
