...
- REQ-6: `gpu_topk_f32` / `gpu_topk_f64` — given a device `[outer, dim]`
  ...
  `CudaSlice<i64>` of `outer * k` original indices into `[0, dim)`, both in
  sorted order. One thread per output slice; serial selection of `k` extrema
  (`largest` → descending value, else ascending; ties broken by ascending
  original index). Mirrors the gather + `sortKeyValueInplace(stable=false)`
  contract of `topk_out_cuda` for the last-dim, sorted case.
- REQ-7: PTX template + ABI for topk. `TOPK_F32_PTX` / `TOPK_F64_PTX`
  (generated by the `topk_ptx!` macro) carry the 7-arg ABI
...
- REQ-9: Dispatch wiring in `CudaBackendImpl::topk_1d`
  ...
  wrapping the value `CudaSlice<V>` via `wrap_slice_{f32,f64}` and the index
  `CudaSlice<i64>` via `wrap_slice_i64`.
- REQ-10: Re-export + non-test production consumer for topk. `pub use
  search::{gpu_topk_f32, gpu_topk_f64}` in `ferrotorch-gpu/src/lib.rs`; the
  production consumer is `ferrotorch-core/src/ops/search.rs::topk`, which on
...
- REQ-21: `gpu_unique_f32` / `gpu_unique_f64` — given a device value buffer of
  `n` elements, return `(CudaBuffer<V> values, Vec<usize> inverse,
  ...
  `unique_init_{f32,f64}_kernel` builds a power-of-2 padded `(key, idx)` pair
  array (`key[i] = i<n ? in[i] : +INF`, `idx[i] = i<n ? i : i32::MAX`); (b) a
  `unique_bitonic_{f32,f64}_kernel` sort-by-key network sorts ascending under a
  ...
  `a != a` iff `a` is NaN), breaking NaN/equal ties by ascending original index;
  ...
  `launch_compact_{f32,f64}` scatters each distinct-value run-start to its unique
...
- REQ-22: PTX + ABI for unique. `UNIQUE_INIT_F32_PTX` / `UNIQUE_INIT_F64_PTX`
  ...
  `UNIQUE_BITONIC_F32_PTX` / `UNIQUE_BITONIC_F64_PTX` (generated by the
  `unique_bitonic_ptx!` macro) carry the 5-arg ABI `(key_ptr, idx_ptr, npad, j,
...
- REQ-23: `GpuBackend::unique_1d` trait surface in
  ...
  dtype { F32, F64 }`, wrapping values via `wrap_buffer{,_f64}`). Re-export +
  non-test production consumer: `pub use search::{gpu_unique_f32, gpu_unique_f64}`
  in `ferrotorch-gpu/src/lib.rs`; the consumer is
  `ferrotorch-core/src/ops/search.rs::unique`, which on CUDA f32/f64 keeps the
...
- [x] AC-25: the SORTED-unique values tensor `is_cuda()`; only the index/run
  ...
  data (`unique_f32_gpu_equals_cpu_finite`). bf16/f16 CUDA inputs reject with
  `NotImplementedOnCuda` (`unique_bf16_cuda_rejects`, `unique_f16_cuda_rejects`).
