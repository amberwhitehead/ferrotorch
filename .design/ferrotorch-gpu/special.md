...
| REQ-7 (Airy Ai unary + Hurwitz zeta binary GPU f32, #1651 GPU tail) | SHIPPED | impl: `pub fn gpu_airy_ai_f32` (carrying `AIRY_AI_F32_PTX`) + `pub fn gpu_zeta_f32` (carrying `ZETA_F32_PTX`, launched via `fn launch_binary_elementwise_f32`) `in ferrotorch-gpu/src/special.rs`, porting `airy_ai_forward` / `zeta` at upstream `aten/src/ATen/native/cuda/Math.cuh:1280-1459, 299-383`. Both loops are BOUNDED over their reachable domains and unroll into flat f32 PTX (airy: a FIXED 36-iter central Maclaurin series, only reached for `x in [-2.09, 8.3203353]`; zeta: a FIXED 9-iter first sum since `while ((i<9)||(a<=9.0))` exits at `i==9`, + a FIXED 12-term Euler-Maclaurin tail, each with a relative-MACHEP `converged` early-exit flag). `pow(a,-x) = ex2.approx.f32(-x*lg2.approx.f32(a))`. Non-test consumer: `CudaBackendImpl::airy_ai_f32` / `zeta_f32 in backend_impl.rs`, dispatched from the `special_gpu_simple` branch of `fn airy_ai` and the `special_gpu_binary` branch of `fn zeta in ferrotorch-core/src/special.rs`. f64 / bf16 / f16 CUDA cleanly reject `NotImplementedOnCuda`; zeta broadcast/mixed-device fall to the CPU `binary_map`. Live-GPU verified by `verify_airy_ai_gpu_f32_*` / `verify_zeta_gpu_f32_*` / `f64_cuda_rejects_not_implemented` / `bf16_f16_cuda_rejects_not_implemented in ferrotorch-gpu/tests/divergence_airy_zeta_gpu_f32.rs` (RTX 3090, torch 2.11.0). |
