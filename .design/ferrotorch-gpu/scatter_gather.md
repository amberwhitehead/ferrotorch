...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `gpu_gather_dim_f32`/`gpu_scatter_dim_f32`/`gpu_scatter_value_dim_f32`/`gpu_scatter_add_dim_f32` in `ferrotorch-gpu/src/scatter_gather_kernels.rs`; non-test consumer: `CudaBackendImpl::{gather,scatter,scatter_value,scatter_add}_dim_f32` overrides in `ferrotorch-gpu/src/backend_impl.rs:10500,10552,10664,10720`, themselves consumed by the `is_cuda()` f32 arm of `ferrotorch_core::ops::indexing::{gather,scatter,scatter_value,scatter_add}` |
