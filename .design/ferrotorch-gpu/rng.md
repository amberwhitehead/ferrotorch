...
| REQ-5 | SHIPPED | impl: `pub(crate) const PHILOX_UNIFORM_PTX` at `rng in rng.rs`, `pub fn gpu_philox_uniform` at `rng in rng.rs`; non-test consumer (#1682): `CudaBackendImpl::rand_uniform_f32 in backend_impl.rs` calls `crate::rng::gpu_philox_uniform`, which `ferrotorch_core::creation::rand_on_device` (Cuda+f32 arm) dispatches to via `GpuBackend::rand_uniform_f32`; production caller `ferrotorch/examples/ferrotorch_bench.rs` GPU-rand bench. Verified on RTX 3090 by `ferrotorch-gpu/tests/on_device_rng.rs`. |
| REQ-6 | SHIPPED | impl: `pub(crate) const PHILOX_NORMAL_PTX` at `rng in rng.rs`, `pub fn gpu_philox_normal` at `rng in rng.rs`; non-test consumer (#1682): `CudaBackendImpl::randn_normal_f32 in backend_impl.rs` calls `crate::rng::gpu_philox_normal`, consumed by `ferrotorch_core::creation::randn_on_device` (Cuda+f32 arm) via `GpuBackend::randn_normal_f32`. Verified on RTX 3090 by `ferrotorch-gpu/tests/on_device_rng.rs`. |
