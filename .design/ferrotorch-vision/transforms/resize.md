...
- REQ-4: SHIPPED — `pub enum InterpolationMode { Nearest, Bilinear }`
  and `Resize::with_interpolation(mode)` add a 2x2-tap bilinear
  sampling path. `antialias`, `max_size`, and shortest-edge int-size
  handling remain follow-ups (bicubic is a future addition; bilinear
  is the immediately impactful gap).
...
- [x] AC-6: `interpolation` (Nearest/Bilinear) shipped (verified by
  ...
  `max_size`, bicubic still NOT-STARTED.
