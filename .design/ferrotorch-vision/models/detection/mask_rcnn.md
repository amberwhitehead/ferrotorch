...
| REQ-6 | SHIPPED | impl: `impl<T> Module<T> for MaskRcnn<T>::forward` in `mask_rcnn.rs` returns first-image scores; non-test consumer: registered as `ModelConstructor<f32>` at `ferrotorch-vision/src/models/registry.rs:285` — the harness/shell consumes it via `Module::forward`. |
