...
| REQ-5 | SHIPPED | impl: `pub static REGISTRY: LazyLock<RwLock<ModelRegistry<f32>>>` + `pub fn list_models` / `pub fn get_model` / `pub fn register_model` (each with poison-handling) in `registry.rs`; non-test consumer: `get_model in examples/inference_dump.rs` calls `ferrotorch_vision::models::get_model(...)`. |
