...
- [x] AC-1: `decoder_forward_shape` test exercises the full forward
  ...
  finiteness (`vae.rs:415..432`).
- [x] AC-2: `vae_decoder_named_parameters_include_post_quant_conv`
  enumerates the canonical key list (`vae.rs:435..449`).
- [x] AC-3: `vae_decoder_decode_with_scaling_matches_manual_div`
  verifies `decode_with_scaling(z) == forward(z / scaling_factor)`
  to 1e-4 (`vae.rs:467..488`).
- [x] AC-4: `round_trip_state_dict` proves the named-parameters and
  `load_state_dict` agree (`vae.rs:491..507`).

## Architecture

- `Decoder<T>` at `vae.rs:30..50` holds the conv pipeline. `new` at
  `vae.rs:51..120` builds: `conv_in` (k=3, pad=1), `UNetMidBlock2D`
...
- `VaeDecoder<T>` at `vae.rs:254..263` wraps a `Decoder<T>` with a
  1×1 `post_quant_conv` over `latent_channels`. `new` at
  `vae.rs:265..288` builds both pieces.
- `decode_with_scaling` at `vae.rs:297..308` computes
...
- `Module<T>::forward` at `vae.rs:314..327` rejects rank-≠-4 or
  wrong-channel inputs, calls `post_quant_conv` then `decoder`.
- `load_state_dict` at `vae.rs:366..389` splits keys into
...
- `ferrotorch-diffusion/src/pipeline.rs:75` `StableDiffusionPipeline`
  holds a `VaeDecoder<T>` field; `pipeline.rs:227` calls
...
- `decoder_forward_shape` (`vae.rs:415..432`) — `[1, 4, 1, 1]` →
  `[1, 3, 8, 8]` on the tiny 4-block config.
- `vae_decoder_forward_shape` (`vae.rs:453..464`) — same with the
  full `VaeDecoder` (incl. `post_quant_conv`).
- `vae_decoder_named_parameters_include_post_quant_conv`
  (`vae.rs:435..449`) — checks canonical keys
  (`post_quant_conv.weight`, `decoder.mid_block.attentions.0.to_q.weight`,
  …) are exposed.
- `vae_decoder_decode_with_scaling_matches_manual_div`
  (`vae.rs:467..488`) — proves the helper agrees with manual scaling.
- `round_trip_state_dict` (`vae.rs:491..507`) — state_dict / load
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `Decoder<T>` at `ferrotorch-diffusion/src/vae.rs:30..50` and `Decoder::new` at `ferrotorch-diffusion/src/vae.rs:51..120`; non-test consumer: `ferrotorch-diffusion/src/vae.rs:281` `VaeDecoder::new` builds it; itself consumed by `ferrotorch-diffusion/src/safetensors_loader.rs:330` `load_vae_decoder` |
| REQ-2 | SHIPPED | impl: `VaeDecoder<T>` at `ferrotorch-diffusion/src/vae.rs:254..263` and `VaeDecoder::new` at `ferrotorch-diffusion/src/vae.rs:265..288`; non-test consumer: `ferrotorch-diffusion/src/safetensors_loader.rs:330` `load_vae_decoder` instantiates and loads it; `ferrotorch-diffusion/src/pipeline.rs:75` carries it as a pipeline field |
| REQ-3 | SHIPPED | impl: `Module<T>::forward` at `ferrotorch-diffusion/src/vae.rs:314..327` (shape check at `vae.rs:316..324`); non-test consumer: `ferrotorch-diffusion/src/pipeline.rs:227` `vae.decode_with_scaling(...)` (which calls `forward` internally) |
