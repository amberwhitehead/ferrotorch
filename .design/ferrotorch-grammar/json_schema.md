...
| REQ-8 | SHIPPED | impl: `pub fn from_compiled`, `vocab_len`, `is_complete`, `grammar(&self) -> &JsonGrammar` accessors in `json_schema.rs`; non-test consumer: `compute_mask_gpu` calls `processor.grammar()` in `ferrotorch-grammar/src/gpu_dispatch.rs:979`. |
