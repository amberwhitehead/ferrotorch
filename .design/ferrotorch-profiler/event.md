...
- `ferrotorch-profiler/src/profiler.rs:5` `use crate::event::{DeviceType, GpuTimingPair, MemoryCategory, ProfileEvent};` — the profiler constructs every kind of event through this re-export.
- `ferrotorch-profiler/src/report.rs:7` `use crate::event::{DeviceType, ProfileEvent};` — `ProfileReport::top_ops` partitions by `DeviceType`, `memory_by_category` groups by `MemoryCategory`.
- `ferrotorch-profiler/src/cuda_timing.rs:45` `use crate::event::{DeviceType, ProfileEvent};` — `PendingCudaScope::finalize` constructs the post-sync GPU event.
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `pub enum DeviceType` at `ferrotorch-profiler/src/event.rs:3` with `Display` at line 12, mirroring `torch/profiler/__init__.py:15` (`from torch._C._autograd import DeviceType`); non-test consumer: `ferrotorch-profiler/src/profiler.rs:125` sets `device_type: DeviceType::Cpu` on every CPU event, `ferrotorch-profiler/src/profiler.rs:215` sets `DeviceType::Cuda` on every GPU event, `ferrotorch-profiler/src/report.rs:150-159` branches on it for per-device aggregation. |
...
| REQ-3 | SHIPPED | impl: `pub struct ProfileEvent` at `ferrotorch-profiler/src/event.rs:58` with the 11 fields documented in source comments, mirroring `torch.autograd.profiler.FunctionEvent`'s public field set (`torch/autograd/profiler.py:494` `table()` columns); non-test consumer: every event-producing method in `ferrotorch-profiler/src/profiler.rs` (`record` at line 116, `record_with_duration` at line 138, `record_memory_categorized` at line 168, `push_gpu_event` at line 205, `OpProfiler::record_op` at line 386), plus `ferrotorch-profiler/src/cuda_timing.rs:179` `PendingCudaScope::finalize` constructs one. |
| REQ-4 | SHIPPED | impl: `pub struct GpuTimingPair` at `ferrotorch-profiler/src/event.rs:98`; non-test consumer: `ferrotorch-profiler/src/profiler.rs:200` `push_gpu_event(timing: GpuTimingPair)` accepts it, computing `duration_us = timing.end_us.saturating_sub(timing.start_us)` at line 209. |
| REQ-5 | SHIPPED | impl: derive attrs at `ferrotorch-profiler/src/event.rs:2` (`DeviceType`), line 24 (`MemoryCategory`), line 57 (`ProfileEvent`), line 97 (`GpuTimingPair`); non-test consumer: `Hash + Eq` on `MemoryCategory` used by `ferrotorch-profiler/src/report.rs:123` `HashMap<MemoryCategory, i64>`; `Clone` on `ProfileEvent` used by `ferrotorch-profiler/src/report.rs:64` `ProfileReport::new(events: Vec<ProfileEvent>)` which clones the slice for `events()` access; `Copy` on `GpuTimingPair` used at `ferrotorch-profiler/src/profiler.rs:497` (test) and consumed by-value at the production call site. |
