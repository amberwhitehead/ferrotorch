...
- `ferrotorch-core/src/grad_fns/arithmetic.rs:388` —
  ...
  Same pattern at `arithmetic.rs:758` (`add_scaled`), `arithmetic.rs:978` (mul),
  `arithmetic.rs:1138` (div), `arithmetic.rs:1272` (neg),
  `arithmetic.rs:1403` (pow), plus `grad_fns/transcendental.rs:107` (exp),
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `pub struct ProfileConfig` at `ferrotorch-profiler/src/profiler.rs:12` with `Default` at line 28, mirroring `torch/autograd/profiler.py:218-222` kwargs; non-test consumer: `ferrotorch-profiler/src/lib.rs:41` re-exports it, `ferrotorch-profiler/src/profiler.rs:437` `with_profiler(config, ...)` consumes it by value, `ferrotorch/src/lib.rs:107` propagates to the meta-crate prelude. |
| REQ-2 | SHIPPED | impl: `pub struct Profiler` at `ferrotorch-profiler/src/profiler.rs:53` with the documented lock-ordering doc at line 47-52; non-test consumer: `ferrotorch-profiler/src/profiler.rs:443` casts `Arc<Profiler>` to `Arc<dyn profiler_hook::OpProfiler>` which requires `Send + Sync`; the cast itself is the proof that the trait bounds hold. |
...
| REQ-4 | SHIPPED | impl: `pub fn with_profiler` at `ferrotorch-profiler/src/profiler.rs:437` with the RAII `ProfilerHookGuard` at line 406, mirroring `torch.profiler.profile.__enter__/__exit__` at `torch/profiler/profiler.py:651`; non-test consumer: `ferrotorch-profiler/src/lib.rs:41` re-exports `with_profiler`, `ferrotorch/src/lib.rs:107` propagates it. The doc-test at `lib.rs:14-25` exercises it as part of `cargo test --doc`. |
| REQ-5 | SHIPPED | impl: `impl profiler_hook::OpProfiler for Profiler` at `ferrotorch-profiler/src/profiler.rs:369` with `record_op` at line 370 populating a `ProfileEvent` from the cross-crate hook signature; non-test consumer: `ferrotorch-core/src/grad_fns/arithmetic.rs:388` `crate::profiler_hook::profile_op_scope("add", "tensor_op", &[a.shape(), b.shape()], || { ... })` invokes `OpProfiler::record_op` on the installed profiler. 36 call sites across `ferrotorch-core/src/grad_fns/{arithmetic,transcendental}.rs`. |
