...
- [ ] AC-14: BatchNorm / InstanceNorm / LocalResponseNorm GPU forward +
  backward — blocker #1449. The current code rejects CUDA inputs with
  `NotImplementedOnCuda` for these layers.
...
**GPU**: BatchNorm currently rejects CUDA inputs with
`NotImplementedOnCuda { op: "BatchNorm2d::forward" }` (and analogously
for 1d/3d/backward). GPU forward + backward dispatch is blocker #1449.
...
CUDA input is rejected with `NotImplementedOnCuda { op:
"LocalResponseNorm::forward" }` — GPU support is part of blocker #1449.

### Non-test production consumers

- `ferrotorch-distributed/src/sync_batch_norm.rs:3,595` —
...
- `ferrotorch-diffusion/src/vae.rs:24,39,99` —
...
- `ferrotorch-bert/src/attention.rs:18,194,209` —
...
- `ferrotorch-whisper/src/encoder.rs:27`, `ferrotorch-whisper/src/layer.rs:12` —
  `use ferrotorch_nn::{Conv1d, GELU, LayerNorm};` — Whisper FFN
  pre/post norm.
- `ferrotorch-nn/src/lazy_norm.rs:14,154-155` — the `LazyBatchNorm{2,3}d`
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `pub struct LayerNorm<T: Float>` and `impl<T: Float> Module<T> for LayerNorm<T>` in `norm.rs`, mirroring `torch/nn/modules/normalization.py:105-238`; non-test consumer: `ferrotorch-bert/src/attention.rs:18,194,209` (`pub layer_norm: LayerNorm<T>`), `ferrotorch-whisper/src/layer.rs:12`. Runner arm: #1447. |
| REQ-2 | SHIPPED | impl: GPU fast path inside `LayerNorm::forward` in `norm.rs` (dispatches to `backend.layernorm_f32(...)` when `input.is_cuda() && self.elementwise_affine`), mirroring `aten/src/ATen/native/Normalization.cpp` (`native_layer_norm_cuda`); non-test consumer: `ferrotorch-bert/src/attention.rs:209` and `ferrotorch-whisper/src/encoder.rs:27` push GPU-resident inputs through this path during inference. |
...
| REQ-9 | SHIPPED | impl: `pub struct InstanceNorm1d<T>`, `pub struct InstanceNorm2d<T>`, `pub struct InstanceNorm3d<T>` newtypes around `InstanceNormInner<T>` + `InstanceNormBackward<T>` in `norm.rs`, mirroring `torch/nn/modules/instancenorm.py`; non-test consumer: `ferrotorch-nn/src/lazy_norm.rs:14` (`use ... InstanceNorm{1,2,3}d`) and `lib.rs:225-228` re-exports. **GPU forward fast path (#1449)**: `InstanceNormInner::forward_impl` routes f32 CUDA input through `backend.group_norm_f32(...)` with `num_groups == num_channels` (InstanceNorm ≡ per-channel GroupNorm), consumed by all three `InstanceNorm{1,2,3}d::forward`. Live GPU↔CPU parity (<1e-4) pinned by `instance_norm2d_forward_gpu_matches_cpu`. |
| REQ-10 | SHIPPED | impl: `pub struct LocalResponseNorm` + `LocalResponseNormBackward<T>` in `norm.rs`, mirroring `torch/nn/modules/normalization.py:16-73`; non-test consumer: `ferrotorch-nn/src/lib.rs:227` re-export. CPU-only — CUDA input returns `NotImplementedOnCuda`; LRN cross-channel-window GPU kernel deferred to a follow-up of #1449 (BatchNorm + InstanceNorm — the common norms — shipped GPU forward this round). |
