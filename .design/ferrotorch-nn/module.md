...
| REQ-2 | SHIPPED | impl: `pub enum Reduction { Mean, Sum, None }` in `module.rs` mirroring `torch/nn/modules/loss.py`'s `reduction` kwarg pattern; non-test consumer: `reduction in ferrotorch-nn/src/loss.rs` `use crate::module::Reduction` (consumed by every loss type) and `reduction in ferrotorch-nn/src/functional.rs` `use crate::module::Reduction`. |
...
| REQ-4 | SHIPPED | impl: `forward` / `parameters` / `parameters_mut` / `named_parameters` as required methods in the trait; non-test consumer: `ferrotorch-optim/src/optimizer.rs` and `ferrotorch-optim/src/adam.rs` consume `.parameters()` via `use ferrotorch_nn::Parameter`. |
