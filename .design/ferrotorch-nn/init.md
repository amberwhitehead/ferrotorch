...
| REQ-3 | SHIPPED | impl: `pub fn uniform`, `pub fn normal` in `init.rs` with xorshift PRNG, mirroring `torch/nn/init.py:247-300`; non-test consumer: `uniform in ferrotorch-nn/src/rnn.rs` (`init::uniform(&mut weight_ih, -k, k)?`) and `torch in ferrotorch-nn/src/embedding.rs` (`init::normal(&mut weight, 0.0, 1.0)?`). |
...
| REQ-5 | SHIPPED | impl: `pub fn kaiming_uniform`, `pub fn kaiming_normal` in `init.rs` (fan_in mode only), mirroring `torch/nn/init.py:554-672`; non-test consumer: re-exported via `init` module + ferrotorch-nn `lib.rs`. Tests `test_kaiming_uniform_relu`, `test_kaiming_normal_relu` validate the std formula. |
...
| REQ-11 | SHIPPED | impl: every initializer rebuilds the parameter via `Parameter::new(Tensor::from_storage(.., true))?` in `init.rs`; non-test consumer: `from_storage in ferrotorch-nn/src/rnn.rs` — calls `init::uniform` and continues using the parameter as a leaf with grad, exercising the preservation contract. Test `test_init_preserves_requires_grad` pins. |
