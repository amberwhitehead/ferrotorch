...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `pub struct FusionGroup` in `dag_fusion.rs`; non-test consumer: re-export at `ferrotorch-jit/src/lib.rs:97` + `ferrotorch-jit/src/codegen.rs:834` iterates `for (i, (group, loops)) in groups.iter().zip(loops_per_group.iter()).enumerate()` reading `group.external_inputs` etc. |
| REQ-2 | SHIPPED | impl: `pub enum FusionGroupKind` in `dag_fusion.rs`; non-test consumer: re-export at `lib.rs:97` + `codegen.rs:1209` `if group.kind != FusionGroupKind::Elementwise { return Ok(None); }` in `try_jit_compile_cpu_rust`. |
| REQ-3 | SHIPPED | impl: `pub fn find_fusion_groups` in `dag_fusion.rs`; non-test consumer: `codegen.rs:823` `let groups = crate::dag_fusion::find_fusion_groups(graph);` (every InductorBackend::generate call) + `codegen.rs:1204` (every InductorBackend::compile CpuRust JIT-path call). |
| REQ-4 | SHIPPED | impl: `pub fn fuse_dag` in `dag_fusion.rs`; non-test consumer: `codegen.rs:824` `let loops_per_group = crate::dag_fusion::fuse_dag(&groups, graph);` + `codegen.rs:1294` `let loops_per_group = fuse_dag(&groups, graph);` in the JIT path. |
...
| REQ-7 | SHIPPED | impl: the external-inputs / external-outputs scan loop at the bottom of `pub fn find_fusion_groups` in `dag_fusion.rs`; non-test consumer: `codegen.rs:836` reads `group.external_inputs.len()` to size the per-kernel input count + `codegen.rs:1247` uses `group.external_inputs` to map runtime sources. |
| REQ-8 | SHIPPED | impl: `fn estimate_numel_for_inputs` + `fn estimate_matmul_dims` in `dag_fusion.rs`; non-test consumer: called by `fn lower_group` (transitively via `fuse_dag` from `codegen.rs:824`). |
