...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `pub type LossFn<T>` at `new in ferrotorch-train/src/learner.rs`; non-test consumer: `Learner::new(..., loss_fn: LossFn<T>)` at line 87 accepts the type; the `Learner.loss_fn` field at line 60 is invoked at `Learner in learner.rs, 420` as `(self.loss_fn)(&output, &target)?`. |
| REQ-2 | SHIPPED | impl: `pub struct Learner<M, T: Float>` at `Learner in ferrotorch-train/src/learner.rs` with all 11 fields; non-test consumer: every method body in `impl<M: Module<T>, T: Float> Learner<M, T>` (lines 79-446) reads/writes these fields directly. |
...
| REQ-7 | SHIPPED | impl: AMP fit-loop branches at `backward_step in ferrotorch-train/src/learner.rs` (`AmpContext::backward_step` path) and `backward_step in ferrotorch-train/src/learner.rs` (standalone `GradScaler` path) both honoured inside `Learner::fit`; non-test consumer: `fit in ferrotorch-train/examples/multi_epoch_train_dump.rs` attaches an `AmpContext` via `Learner::with_amp_context` and runs `Learner::fit` end-to-end on every default invocation of the binary, exercising the AMP backward path (closes #1500). |
| REQ-8 | SHIPPED | impl: accessors `model()` at `model in ferrotorch-train/src/learner.rs`, `model_mut()` at `model_mut in ferrotorch-train/src/learner.rs`, `epoch()` at `epoch in ferrotorch-train/src/learner.rs`, `step()` at `step in ferrotorch-train/src/learner.rs`, `skipped_steps()` at `skipped_steps in ferrotorch-train/src/learner.rs`; non-test consumer: `examples/multi_epoch_train_dump.rs` reads `learner.model()` to snapshot parameter state after each epoch via the model's `state_dict()`. |
