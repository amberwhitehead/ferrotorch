...
| REQ-6 | NOT-STARTED | open prereq blocker #1499 — `load_checkpoint` impl at `ferrotorch-train/src/learner.rs:195-202` exists, but the only call site is the unit-test mock. No out-of-tree production caller (e.g. `examples/multi_epoch_train_dump.rs` does not resume-from-disk; a resume-from-disk binary or downstream model crate is the open consumer). |
| REQ-7 | NOT-STARTED | open prereq blocker #1500 — AMP fit-loop branch at `ferrotorch-train/src/learner.rs:272-281` is implemented, but no production caller attaches a `GradScaler` to a real fit run. The construction surface (`with_grad_scaler` + the field) is exercised by unit tests; the end-to-end fit-loop AMP path needs a CUDA-resident-model example as its production consumer. |
