...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: 8 `pub mod` declarations at `ferrotorch-train/src/lib.rs`; non-test consumer: `ferrotorch-train/src/learner.rs` (`use crate::callback::Callback;` etc.), `ferrotorch-train/src/tensorboard.rs`, and `Callback in ferrotorch-train/src/callback.rs` — cross-submodule wiring exercises every module declaration. |
...
| REQ-3 | SHIPPED | impl: `#![warn(clippy::all, clippy::pedantic)]` + `#![deny(rust_2018_idioms)]` at `ferrotorch-train/src/lib.rs`, followed by per-lint `#![allow]` block at `lib.rs` with one-line rationale per allow; non-test consumer: every submodule under `ferrotorch-train/src/` inherits the baseline via the `mod` declarations — clippy clean at `-D warnings` is the integration test. |
| REQ-4 | SHIPPED | impl: fenced doctest at `ferrotorch-train/src/lib.rs` constructs `LossMetric`/`EarlyStopping`/`ProgressLogger`/`EmaCallback`/`TrainingHistory` with no model/dataset dependency; non-test consumer: the doctest is the consumer — it compiles and runs against the public surface as the user would, and is wired into `cargo test -p ferrotorch-train --doc`. |
