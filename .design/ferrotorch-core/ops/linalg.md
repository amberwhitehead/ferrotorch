...
| REQ-6 (mm_raw_bt) | SHIPPED | impl: `pub fn mm_raw_bt in ops/linalg.rs` (fused `A @ B^T`). Non-test production consumers: `grad_fns/linalg.rs:129 let result = crate::ops::linalg::mm_raw_bt(gc_data, b_data, m, n, k);` inside `MmBackward::backward`, `grad_fns/linalg.rs:1026 let result_vec = linalg::mm_raw_bt(a_data, b_data, m, k, n);`, and `pub fn linear_fused in grad_fns/linalg.rs` (`let mut result_vec = linalg::mm_raw_bt(a_data, w_data, m, k, n);`). Mirrors `gemm_transb_` family at `aten/src/ATen/native/cpu/BlasKernel.cpp:199-308`. |
