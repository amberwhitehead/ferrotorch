...
- REQ-7: `mm_raw_at(a_data, b_data, m, k, n)` — fused `A^T @ B` on raw
  ...
  backward of `mm` for `dB = A^T @ grad_C` at `grad_fns/linalg.rs:145`.
...
| REQ-7 (mm_raw_at) | SHIPPED | impl: `pub fn mm_raw_at in ops/linalg.rs` (fused `A^T @ B`). Non-test production consumers: `grad_fns/linalg.rs:145 let result = crate::ops::linalg::mm_raw_at(a_data, gc_data, k, m, n);` inside `MmBackward::backward`, `grad_fns/linalg.rs:953 let result = crate::ops::linalg::mm_raw_at(gc_data, a_data, n, m, k);`, and `grad_fns/linalg.rs:1155 let result = crate::ops::linalg::mm_raw_at(gc_data, a_data, n, m, k);`. Mirrors `gemm_transa_` family at `aten/src/ATen/native/cpu/BlasKernel.cpp:171-197`. |
