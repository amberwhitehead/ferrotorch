...
- [x] AC-9: GPU path for `topk` (f32/f64/f16/bf16, last-dim, largest/smallest,
  ...
  stays GPU-resident and only the int64 indices are read back. `meshgrid` and
  `unique_consecutive` now lower f32/f64/f16/bf16 on-device via
  `GpuBackend`; `unique`, `histc`, and arbitrary non-last-dim `topk` still
  have narrower CUDA dtype/axis coverage tracked under #1545 follow-ups.
