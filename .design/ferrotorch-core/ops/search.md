...
- REQ-4: `unique_consecutive(input)` — same return tuple but
  ...
  `torch.unique_consecutive`. CUDA f32/f64/f16/bf16 run the data-dependent run
...
- REQ-7: `topk(input, k, largest)` / `topk_dim(input, k, dim, largest)` /
  `topk_dim_sorted(input, k, dim, largest, sorted)` — return `(values,
  indices)` for the k largest (or smallest) along the selected dim. The values
  shape is the input shape with that dim replaced by `k`; the host index vector
  is the same row-major logical index tensor flattened, with entries indexing
  the selected input dimension. Mirrors `torch.topk`. CUDA f32/f64/f16/bf16
  inputs lower on-device via `GpuBackend::topk_nd` (#1545): the k-selection
  runs on the GPU over the row-major `[outer, dim, inner]` decomposition, the
  VALUES tensor stays GPU-resident, and only the int64 indices are read back to
  host. Ties resolve to ascending original index (a valid `torch.topk` result;
  matches the CPU stable-sort path). The PyTorch 0-D special case is preserved:
  `k == 0` and `k == 1` both return the scalar value and scalar index 0, while
  `k > 1` errors.

## Acceptance Criteria

- [x] AC-1: `cargo test -p ferrotorch-core --lib ops::search`
  passes, covering each operation.
...
- [x] AC-7: `topk_dim(input, k>selected_dim)` errors with `InvalidArgument`;
  scalar `topk_dim(k=0|1, dim=0|-1)` returns scalar value + index 0 and
  `k>1` errors, matching torch.
...
- [x] AC-9: GPU path for `topk` / `topk_dim` (f32/f64/f16/bf16, arbitrary dim,
  largest/smallest, sorted values) — SHIPPED (#1545). CUDA inputs lower the
  k-selection on-device via `GpuBackend::topk_nd` (`ferrotorch-gpu/src/search.rs`);
  the values tensor stays GPU-resident and only the int64 indices are read back.
  `meshgrid`, `unique`, and `unique_consecutive` lower f32/f64/f16/bf16
  on-device via `GpuBackend`; `histc` lowers f32/f64.
...
`topk` delegates to `topk_dim(input, k, -1, largest)`.

`topk_dim` / `topk_dim_sorted`:
1. Normalize `dim`, with the torch 0-D special case handled before the general
   ndim path.
2. Validate `k <= shape[dim]`; if the selected dimension is zero, only `k == 0`
   reaches the shaped-empty fast path.
3. Decompose the contiguous logical layout as `[outer, dim_size, inner]`, where
   `outer = product(shape[..dim])` and `inner = product(shape[dim+1..])`.
4. CPU: for each `(outer, inner)` lane, gather the selected-dim slice, run the
   torch CPU topk selection helper, and write values/indices to row-major
   `[outer, k, inner]`.
5. CUDA f32/f64/f16/bf16: materialize a contiguous CUDA input when needed, call
   `GpuBackend::topk_nd(values, outer, dim_size, inner, k, largest)`, wrap the
   returned value handle as a CUDA `Tensor`, and read back only the i64 index
   handle to satisfy the public host-`Vec<usize>` API.
6. Output shape = input shape with selected dim -> `k`. The values tensor
   carries `TopkBackward`, which scatters incoming cotangents back along the
   selected dimension.
...
clamping) paths. `cargo test -p ferrotorch-core --test audit_core_topk_dim`
pins the PyTorch-probed arbitrary-dim forward, scalar special case, and
selected-axis backward scatter. CUDA residency and arbitrary-dim PTX behavior
are covered by the ferrotorch-gpu topk tests.
...
| REQ-3 | SHIPPED | impl: `unique in ops/search.rs`; non-test consumer: re-exported as `ferrotorch_core::unique`. CUDA f32/f64/f16/bf16 lower the SORTED sort-by-key dedup on-device via `GpuBackend::unique_1d` (#1545); GPU impl `gpu_unique_f32`/`_f64`/`_f16`/`_bf16 in ferrotorch-gpu/src/search.rs` (bitonic sort-by-key + run-flag/prefix-sum/compaction, mirroring `aten/src/ATen/native/cuda/Unique.cu:51-85`), GPU consumer `CudaBackendImpl::unique_1d in ferrotorch-gpu/src/backend_impl.rs`; SORTED-unique values stay GPU-resident, only index/run metadata read back. NaN NOT collapsed (each distinct, sorted last) and finite equal signed-zero representatives keep the last original payload — matches live torch 2.11. |
| REQ-4 | SHIPPED | impl: `unique_consecutive in ops/search.rs`; non-test consumer: re-exported as `ferrotorch_core::unique_consecutive`. CUDA f32/f64/f16/bf16 lower the run compaction on-device via `GpuBackend::unique_consecutive_1d` (#1545); GPU impl `gpu_unique_consecutive_f32`/`_f64`/`_f16`/`_bf16 in ferrotorch-gpu/src/search.rs`, GPU consumer `CudaBackendImpl::unique_consecutive_1d in ferrotorch-gpu/src/backend_impl.rs`; values stay GPU-resident, only run-position metadata read back. |
...
| REQ-6 | SHIPPED | impl: `meshgrid` + `meshgrid_indexing in ops/search.rs` (`MeshIndexing::Xy` swaps first two inputs+grids per `TensorShape.cpp:4433-4438,4470-4472`); non-test consumer: `meshgrid` delegates to `meshgrid_indexing`, both re-exported as `ferrotorch_core::{meshgrid, meshgrid_indexing}`. CUDA f32/f64/f16/bf16 via `GpuBackend::meshgrid_grid` (#1545), preserving dtype/device with no value round trip. |
| REQ-7 | SHIPPED | impl: `topk`, `topk_dim`, and `topk_dim_sorted in ops/search.rs`; non-test consumer: re-exported as `ferrotorch_core::{topk, topk_dim, topk_dim_sorted}`. CUDA f32/f64/f16/bf16 arbitrary-dim values via `GpuBackend::topk_nd` (#1545), values stay GPU-resident and only int64 indices are read back. |
