...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `QuantScheme` at `ferrotorch-core/src/quantize.rs:21`, `QuantDtype` at `:30`; non-test consumer: re-exported at `ferrotorch-core/src/lib.rs:179-181`, reachable by downstream callers via `ferrotorch_core::QuantDtype::Int8`. |
| REQ-2 | SHIPPED | impl: `QuantizedTensor` at `ferrotorch-core/src/quantize.rs:73`; non-test consumer: re-exported at `lib.rs:179-181`; threaded through `quantize_named_tensors`'s return type and bulk state-dict callers. |
| REQ-3 | SHIPPED | impl: `quantize` at `ferrotorch-core/src/quantize.rs:222`; non-test consumer: re-exported at `lib.rs:179-181`; called by `quantize_named_tensors` at `:478` and by `FakeQuantize::forward` (transitively, via `grad_fns/quantize_grad.rs:283`). |
| REQ-4 | SHIPPED | impl: `dequantize` at `ferrotorch-core/src/quantize.rs:333`; non-test consumer: re-exported at `lib.rs:179-181`; called by `quantized_matmul` at `:372` and by `FakeQuantize::forward`. |
...
| REQ-6 | SHIPPED | impl: `QParams` at `ferrotorch-core/src/quantize.rs:497`; non-test consumer: re-exported at `lib.rs:179-181`; produced by every observer and consumed by `QatModel.step()`. |
| REQ-7 | SHIPPED | impl: `trait Observer` at `ferrotorch-core/src/quantize.rs:544`, `MinMaxObserver` at `:561`, `PerChannelMinMaxObserver` at `:616`, `HistogramObserver` at `:735`; non-test consumer: re-exported at `lib.rs:179-181`; threaded through `QatLayer` at `:967`. |
| REQ-8 | SHIPPED | impl: `FakeQuantize` at `ferrotorch-core/src/quantize.rs:878`; non-test consumer: `grad_fns/quantize_grad.rs:283` (`FakeQuantizeBackward` attaches via the forward op) — production autograd graph. Consumer chain `Tensor::fake_quantize_per_tensor_affine_t` at `methods.rs:596`. |
| REQ-9 | SHIPPED | impl: `QatLayer` at `ferrotorch-core/src/quantize.rs:967`, `QatModel` at `:981`, `prepare_qat` at `:1057`; non-test consumer: re-exported at `lib.rs:179-181`; entrypoint for the QAT training-loop API. |
