...
| REQ-4 | SHIPPED | impl: `reduce_all` at `ferrotorch-core/src/meta_propagate.rs:109`; non-test consumer: `sum in grad_fns/reduction.rs` (the `Tensor::sum_all` full reduction), `mean in grad_fns/reduction.rs` (the `Tensor::mean_all` full reduction), `prod in grad_fns/reduction.rs` (the `Tensor::prod_all` full reduction) each call `reduce_all` before their data kernel. |
