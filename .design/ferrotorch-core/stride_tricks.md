...
| REQ-5 | SHIPPED | impl: `AsStridedBackward` at `ferrotorch-core/src/stride_tricks.rs:336` with `impl GradFn::backward` at `:360`; non-test consumer: `Tensor::as_strided` attaches it when `requires_grad` is set (`:213-219`) — production autograd graph routes through this when stride ops appear in a differentiable function. |
