...
Leaf gradient hooks fire in `accumulate_leaf_grad` at `graph.rs:99-111`
through `run_tensor_grad_hooks` at `graph.rs:115-131`. Post-accumulate
hooks fire at `graph.rs:108-111` after the leaf's `.grad()` is written.

### REQ-9 — parallel backward

`pub fn backward_parallel` at `graph.rs:415-658` reuses the Phase-1
...
`:676-746` for non-leafs), and decrements input in-degrees with
...
error wins. Recording an error sets the cancellation flag and calls
`notify_parallel_waiters` at `graph.rs:232-240`, which takes the same
ready-queue mutex used by waiters before notifying. Completion uses the
same helper, preventing lost wake-ups between a worker's final
cancelled/processed check and `Condvar::wait`.
...
| REQ-4 | SHIPPED | impl: `accumulate_non_leaf_grad` at `graph.rs:762-873` (sequential), `accumulate_non_leaf_grad_locked` at `:676-746` (parallel), and same-shape/device guard `validate_non_leaf_grad` at `:875-902`; non-test consumer: invoked from inside REQ-1/REQ-2 dispatch; the parallel variant invoked from REQ-9's parallel engine; production path: `Tensor::backward` and `Tensor::backward_with_gradient`. |
| REQ-5 | SHIPPED | impl: `run_grad_hooks` + `run_post_accumulate_hooks` calls through `accumulate_leaf_grad` at `graph.rs:99-111`, reached by sequential and parallel engines; mirrors PyTorch's hook chain in `torch/utils/hooks.py:93+`; non-test production consumer: every leaf tensor with `register_hook` registered via `Tensor::register_hook` at `register_hook in ferrotorch-core/src/tensor.rs` flows through this path during user `loss.backward()` calls. |
...
| REQ-13 | SHIPPED | impl: `lock_parallel_state`, `wait_parallel_state`, `record_parallel_error`, `take_parallel_error`, `notify_parallel_waiters`, and the `catch_unwind` worker boundary in `backward_parallel`; mirrors PyTorch `torch/csrc/autograd/engine.cpp` `thread_main` / `thread_on_exception` / `GraphTask::set_exception`; tests: `test_backward_parallel_node_error_returns_structured_error`, `test_backward_parallel_worker_panic_returns_structured_error`, and `audit_autograd_engine_utilities::backward_parallel_error_path_returns_before_timeout`. |
