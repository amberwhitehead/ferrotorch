...
| REQ-4 | SHIPPED | impl: `pub fn is_f32` / `is_f64` / `is_bf16` / `is_f16` at `ferrotorch-core/src/dtype_dispatch.rs:126/:133/:140/:149`. Non-test production consumer: `ferrotorch-core/src/fft.rs:139` checks `if input.is_cuda() && (is_f32::<T>() || is_f64::<T>())` to gate the GPU FFT path; `ferrotorch-core/src/grad_fns/arithmetic.rs:119` uses `if is_f32::<T>()` to pick the right helper. |
