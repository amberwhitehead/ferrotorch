...
- REQ-10: `rsqrt(a)` — `torch.rsqrt(input)` is `1 / sqrt(input)`. Per
  ...
  `arithmetic::rsqrt` at `grad_fns/arithmetic.rs:1784` (dedicated forward +
...
- REQ-11: `reciprocal(a)` — `torch.reciprocal(input)` is `1 / input`. Per
  ...
  `arithmetic::reciprocal` at `grad_fns/arithmetic.rs:1932` (dedicated
...
- REQ-12: `floor_divide(a, b)` — `torch.floor_divide(input, other)` floors
  ...
  `grad_fns/arithmetic.rs:2769` (CPU kernel mirroring
...
- REQ-13: `remainder(a, b)` — `torch.remainder(input, other)` returns the
  ...
  SHIPPED via `arithmetic::remainder` at `grad_fns/arithmetic.rs:2132`
...
- REQ-14: `fmod(a, b)` — `torch.fmod(input, other)` returns the remainder
  ...
  SHIPPED via `arithmetic::fmod` at `grad_fns/arithmetic.rs:2439` (CPU
...
- REQ-15: `addcmul(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcmul` at `grad_fns/arithmetic.rs:3116`
...
- REQ-16: `addcdiv(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcdiv` at `grad_fns/arithmetic.rs:3431`
...
- [x] AC-10: `rsqrt` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::rsqrt` (`grad_fns/arithmetic.rs:1784`) + the dedicated
...
- [x] AC-11: `reciprocal` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::reciprocal` (`grad_fns/arithmetic.rs:1932`) + the dedicated
...
- [x] AC-13: `remainder` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::remainder` (`grad_fns/arithmetic.rs:2132`) + the dedicated
...
- [x] AC-14: `fmod` parity-sweep at `--seeds 8` returns
  ...
  #1199 via `arithmetic::fmod` (`grad_fns/arithmetic.rs:2439`) +
...
- [x] AC-12: `floor_divide` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::floor_divide` (`grad_fns/arithmetic.rs:2769`) +
...
- [x] AC-15: `addcmul` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::addcmul` (`grad_fns/arithmetic.rs:3116`) + the dedicated
...
- [x] AC-16: `addcdiv` parity-sweep at `--seeds 8` returns
  ...
  #1201 via `arithmetic::addcdiv` (`grad_fns/arithmetic.rs:3431`) +
...
`(reduce(grad, a.shape()), reduce(grad, b.shape()))` (`arithmetic.rs:350-373`).
The forward `pub fn add` (`arithmetic.rs:375-391`) emits a profiler scope,
checks device-match, lets `meta_propagate::binary_broadcast` short-circuit
for meta tensors, then calls `add_inner`. `add_inner` (`arithmetic.rs:392-488`)
...
`AddScaledBackward` (`arithmetic.rs:497-538`) saves `a`, `b`, and `alpha:
f64`; backward returns `(reduce(grad, a.shape()), reduce(alpha*grad,
b.shape()))`. `scale_tensor` (`arithmetic.rs:559-591`) is a private helper
that routes to dtype-specialised GPU `scale_*` kernels (f32/f64/bf16/f16)
or to `scalar_map` on CPU. `check_out_allowed` (`arithmetic.rs:624-648`)
enforces torch's `out=` rules: no grad_fn, no requires_grad-leaf. `add_out`
(`arithmetic.rs:652-691`) is an `alpha=1.0` wrapper over `add_scaled_out`.
`add_scaled_out` (`arithmetic.rs:697-768`) validates devices, computes
...
proof. `add_scaled` (`arithmetic.rs:773-931`) shortcircuits `alpha == 1.0`
...
- **Fused single-launch GPU path (#1675)**: `maybe_add_scaled_fused_gpu`
  (`arithmetic.rs:876`) returns `Some(result)` when `a` and `b` are both
...
`MulBackward` (`arithmetic.rs:1035-1090`) saves `a`/`b`; backward has two
...
(`arithmetic.rs:1094-1251`) is structurally identical to `add`/`add_inner`
...
`DivBackward` (`arithmetic.rs:1210-1244`) — `da = grad / b`,
`db = -grad * a / (b*b)`. Forward `div`/`div_inner`
(`arithmetic.rs:1254-1413`) — `dispatch_floating_dtype!` over
...
`NegBackward` (`arithmetic.rs:1372-1393`) returns `-grad` under `no_grad`.
Forward `neg`/`neg_inner` (`arithmetic.rs:1396-1488`) routes through
...
`PowBackward` (`arithmetic.rs:1447-1524`) saves `a` + `exp: f64`; backward
...
`pow`/`pow_inner` (`arithmetic.rs:1526-1615`) uses
...
`SqrtBackward` (`arithmetic.rs:1574-1622`) — `grad / (2 * sqrt(a))`. GPU
...
direct zip-map. Forward `sqrt`/`sqrt_inner` (`arithmetic.rs:1628-1693`)
...
`AbsBackward` (`arithmetic.rs:3564-3629`) — `grad * sign(a)` with explicit
...
`abs`/`abs_inner` (`arithmetic.rs:3635-3677`) uses
...
`rsub` (`arithmetic.rs:1020`) is a one-line wrapper over `sub_scaled(b, a,
...
`RemainderBackward` (`arithmetic.rs:2018-2061`) saves `a`/`b`; backward
...
`FmodBackward` (`arithmetic.rs:2321-2364`) saves `a`/`b`; backward
...
Forward `pub fn floor_divide` (`arithmetic.rs:2769-2788`) emits a
profiler scope, checks device match, lets
...
`AddcmulBackward` (`arithmetic.rs:2973-3071`) saves `input` / `tensor1`
...
Forward `pub fn addcmul` (`arithmetic.rs:3116-3143`) emits a profiler
...
`AddcdivBackward` (`arithmetic.rs:3269-3368`) saves `input` / `tensor1`
...
Forward `pub fn addcdiv` (`arithmetic.rs:3431-3457`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
...
full impl-and-consumer evidence chain (`arithmetic.rs:2769` + `:2483` +
