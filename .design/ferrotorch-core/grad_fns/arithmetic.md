...
- REQ-9: `rsub(a, b, *, alpha=1)` — `torch.rsub(input, other, *, alpha=1)`
  ...
  `arithmetic::rsub` at `grad_fns/arithmetic.rs:1021` (one-line wrapper
...
- REQ-10: `rsqrt(a)` — `torch.rsqrt(input)` is `1 / sqrt(input)`. Per
  ...
  `arithmetic::rsqrt` at `grad_fns/arithmetic.rs:1785` (dedicated forward +
...
- REQ-11: `reciprocal(a)` — `torch.reciprocal(input)` is `1 / input`. Per
  ...
  `arithmetic::reciprocal` at `grad_fns/arithmetic.rs:1933` (dedicated
...
- REQ-12: `floor_divide(a, b)` — `torch.floor_divide(input, other)` floors
  ...
  `grad_fns/arithmetic.rs:2770` (CPU kernel mirroring
...
- REQ-13: `remainder(a, b)` — `torch.remainder(input, other)` returns the
  ...
  SHIPPED via `arithmetic::remainder` at `grad_fns/arithmetic.rs:2133`
...
- REQ-14: `fmod(a, b)` — `torch.fmod(input, other)` returns the remainder
  ...
  SHIPPED via `arithmetic::fmod` at `grad_fns/arithmetic.rs:2440` (CPU
...
- REQ-15: `addcmul(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcmul` at `grad_fns/arithmetic.rs:3117`
...
- REQ-16: `addcdiv(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcdiv` at `grad_fns/arithmetic.rs:3432`
...
- [x] AC-9: `rsub` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::rsub` (`grad_fns/arithmetic.rs:1021`) delegating to
...
- [x] AC-10: `rsqrt` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::rsqrt` (`grad_fns/arithmetic.rs:1785`) + the dedicated
...
- [x] AC-11: `reciprocal` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::reciprocal` (`grad_fns/arithmetic.rs:1933`) + the dedicated
...
- [x] AC-13: `remainder` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::remainder` (`grad_fns/arithmetic.rs:2133`) + the dedicated
...
- [x] AC-14: `fmod` parity-sweep at `--seeds 8` returns
  ...
  #1199 via `arithmetic::fmod` (`grad_fns/arithmetic.rs:2440`) +
...
- [x] AC-12: `floor_divide` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::floor_divide` (`grad_fns/arithmetic.rs:2770`) +
...
- [x] AC-15: `addcmul` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::addcmul` (`grad_fns/arithmetic.rs:3117`) + the dedicated
...
- [x] AC-16: `addcdiv` parity-sweep at `--seeds 8` returns
  ...
  #1201 via `arithmetic::addcdiv` (`grad_fns/arithmetic.rs:3432`) +
...
- [x] AC-17: `cargo test -p ferrotorch-core --lib grad_fns::arithmetic`
  passes (the `tests` mod at `arithmetic.rs:3678-4021` covers forward,
...
(`arithmetic.rs:653-691`) is an `alpha=1.0` wrapper over `add_scaled_out`.
`add_scaled_out` (`arithmetic.rs:698-768`) validates devices, computes
...
proof. `add_scaled` (`arithmetic.rs:774-931`) shortcircuits `alpha == 1.0`
...
- **Fused single-launch GPU path (#1675)**: `maybe_add_scaled_fused_gpu`
  (`arithmetic.rs:877`) returns `Some(result)` when `a` and `b` are both
...
- `pub fn sub<T: Float>(a, b)` at `arithmetic.rs:940` is a one-line
  delegation: `sub_scaled(a, b, 1.0)`.
- `pub fn sub_scaled<T: Float>(a, b, alpha: f64)` at `arithmetic.rs:969`
...
`MulBackward` (`arithmetic.rs:1036-1090`) saves `a`/`b`; backward has two
...
(`arithmetic.rs:1095-1251`) is structurally identical to `add`/`add_inner`
...
`DivBackward` (`arithmetic.rs:1211-1244`) — `da = grad / b`,
`db = -grad * a / (b*b)`. Forward `div`/`div_inner`
(`arithmetic.rs:1255-1413`) — `dispatch_floating_dtype!` over
...
`NegBackward` (`arithmetic.rs:1373-1393`) returns `-grad` under `no_grad`.
Forward `neg`/`neg_inner` (`arithmetic.rs:1397-1488`) routes through
...
`PowBackward` (`arithmetic.rs:1448-1524`) saves `a` + `exp: f64`; backward
...
`pow`/`pow_inner` (`arithmetic.rs:1527-1615`) uses
...
`SqrtBackward` (`arithmetic.rs:1575-1622`) — `grad / (2 * sqrt(a))`. GPU
...
direct zip-map. Forward `sqrt`/`sqrt_inner` (`arithmetic.rs:1629-1693`)
...
`AbsBackward` (`arithmetic.rs:3565-3629`) — `grad * sign(a)` with explicit
...
`abs`/`abs_inner` (`arithmetic.rs:3636-3677`) uses
...
`rsub` (`arithmetic.rs:1021`) is a one-line wrapper over `sub_scaled(b, a,
...
`RemainderBackward` (`arithmetic.rs:2019-2061`) saves `a`/`b`; backward
...
Forward `pub fn remainder` (`arithmetic.rs:2133`) emits a profiler
...
`FmodBackward` (`arithmetic.rs:2322-2364`) saves `a`/`b`; backward
...
treatment). Forward `pub fn fmod` (`arithmetic.rs:2440`) emits a
...
`FloorDivideBackward` (`arithmetic.rs:2613-2649`) saves `a`/`b`;
...
Forward `pub fn floor_divide` (`arithmetic.rs:2770-2788`) emits a
profiler scope, checks device match, lets
...
`AddcmulBackward` (`arithmetic.rs:2974-3071`) saves `input` / `tensor1`
...
Forward `pub fn addcmul` (`arithmetic.rs:3117-3143`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
tensor2), and calls `addcmul_inner`. The inner (`arithmetic.rs:3144-
...
`AddcdivBackward` (`arithmetic.rs:3270-3368`) saves `input` / `tensor1`
...
Forward `pub fn addcdiv` (`arithmetic.rs:3432-3457`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
tensor2), and calls `addcdiv_inner`. The inner (`arithmetic.rs:3459-
...
### Unit tests (in-file `#[cfg(test)] mod tests` at `arithmetic.rs:3678-5449`)
...
full impl-and-consumer evidence chain (`arithmetic.rs:2770` + `:2484` +
