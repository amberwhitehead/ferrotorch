...
`(reduce(grad, a.shape()), reduce(grad, b.shape()))` (`arithmetic.rs:482-509`).
The forward `pub fn add` (`arithmetic.rs:512`) emits a profiler scope,
checks device-match, lets `meta_propagate::binary_broadcast` short-circuit
for meta tensors, then calls `add_inner`. `add_inner` (`arithmetic.rs:529-625`)
...
`AddScaledBackward` (`arithmetic.rs:634-687`) saves `a`, `b`, and `alpha:
f64`; backward returns `(reduce(grad, a.shape()), reduce(alpha*grad,
b.shape()))`. `scale_tensor` (`arithmetic.rs:696-727`) is a private helper
that routes to dtype-specialised GPU `scale_*` kernels (f32/f64/bf16/f16)
or to `scalar_map` on CPU. `check_out_allowed` (`arithmetic.rs:765-804`)
enforces torch's `out=` rules: no grad_fn, no requires_grad-leaf. `add_out`
(`arithmetic.rs:823-832`) is an `alpha=1.0` wrapper over `add_scaled_out`.
`add_scaled_out` (`arithmetic.rs:873-942`) validates devices, computes
...
proof. `add_scaled` (`arithmetic.rs:950-1035`) shortcircuits `alpha == 1.0`
...
`MulBackward` (`arithmetic.rs:1212-1268`) saves `a`/`b`; backward has two
...
(`arithmetic.rs:1271-1365`) is structurally identical to `add`/`add_inner`
...
`DivBackward` (`arithmetic.rs:1387-1443`) — `da = grad / b`,
`db = -grad * a / (b*b)`. Forward `div`/`div_inner`
(`arithmetic.rs:1449-1545`) — `dispatch_floating_dtype!` over
...
`NegBackward` (`arithmetic.rs:1567-1587`) returns `-grad` under `no_grad`.
Forward `neg`/`neg_inner` (`arithmetic.rs:1591-1627`) routes through
...
`PowBackward` (`arithmetic.rs:1642-1753`) saves `a` + `exp: f64`; backward
...
`pow`/`pow_inner` (`arithmetic.rs:1759-1808`) uses
...
`SqrtBackward` (`arithmetic.rs:1821-1875`) — `grad / (2 * sqrt(a))`. GPU
...
`RemainderBackward` (`arithmetic.rs:2275-2326`) saves `a`/`b`; backward
...
`FmodBackward` (`arithmetic.rs:2583-2629`) saves `a`/`b`; backward
...
Forward `pub fn floor_divide` (`arithmetic.rs:3031-3049`) emits a
profiler scope, checks device match, lets
...
full impl-and-consumer evidence chain (`arithmetic.rs:3031` + `:2874` +
