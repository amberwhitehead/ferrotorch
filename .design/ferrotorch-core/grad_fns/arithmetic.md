...
- REQ-9: `rsub(a, b, *, alpha=1)` — `torch.rsub(input, other, *, alpha=1)`
  ...
  `arithmetic::rsub` at `grad_fns/arithmetic.rs:1875` (one-line wrapper
...
- REQ-10: `rsqrt(a)` — `torch.rsqrt(input)` is `1 / sqrt(input)`. Per
  ...
  `arithmetic::rsqrt` at `grad_fns/arithmetic.rs:2771` (dedicated forward +
...
- REQ-11: `reciprocal(a)` — `torch.reciprocal(input)` is `1 / input`. Per
  ...
  `arithmetic::reciprocal` at `grad_fns/arithmetic.rs:2926` (dedicated
...
- REQ-12: `floor_divide(a, b)` — `torch.floor_divide(input, other)` floors
  ...
  `grad_fns/arithmetic.rs:3706` (CPU kernel and CUDA broadcast kernels
  mirror `c10::div_floor_floating`: `fmod`-then-`(a-mod)/b` Python
  `__floordiv__` form with the `(b<0)!=(mod<0)` sign-correction
...
- REQ-13: `remainder(a, b)` — `torch.remainder(input, other)` returns the
  ...
  SHIPPED via `arithmetic::remainder` at `grad_fns/arithmetic.rs:3124`
  (CPU kernel plus CUDA f32/f64/f16/bf16 broadcast kernels matching
  `aten/src/ATen/native/cpu/BinaryOpsKernel.cpp:398-401`'s
  `fmod`-then-correct branch: `scalar_t mod = std::fmod(a, b); if
  ((mod != 0) && ((b < 0) != (mod < 0))) mod += b;` — equivalent
...
- REQ-14: `fmod(a, b)` — `torch.fmod(input, other)` returns the remainder
  ...
  SHIPPED via `arithmetic::fmod` at `grad_fns/arithmetic.rs:3402`
  (CPU kernel plus CUDA f32/f64/f16/bf16 broadcast kernels matching
...
- REQ-15: `addcmul(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcmul` at `grad_fns/arithmetic.rs:4097`
...
- REQ-16: `addcdiv(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcdiv` at `grad_fns/arithmetic.rs:4452`
...
- [x] AC-9: `rsub` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::rsub` (`grad_fns/arithmetic.rs:1875`) delegating to
...
- [x] AC-10: `rsqrt` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::rsqrt` (`grad_fns/arithmetic.rs:2771`) + the dedicated
...
- [x] AC-11: `reciprocal` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::reciprocal` (`grad_fns/arithmetic.rs:2926`) + the dedicated
...
- [x] AC-13: `remainder` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::remainder` (`grad_fns/arithmetic.rs:3124`) + the dedicated
...
- [x] AC-14: `fmod` parity-sweep at `--seeds 8` returns
  ...
  #1199 via `arithmetic::fmod` (`grad_fns/arithmetic.rs:3402`) +
...
- [x] AC-12: `floor_divide` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::floor_divide` (`grad_fns/arithmetic.rs:3706`) +
...
- [x] AC-15: `addcmul` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::addcmul` (`grad_fns/arithmetic.rs:4097`) + the dedicated
...
- [x] AC-16: `addcdiv` parity-sweep at `--seeds 8` returns
  ...
  #1201 via `arithmetic::addcdiv` (`grad_fns/arithmetic.rs:4452`) +
...
- [x] AC-17: `cargo test -p ferrotorch-core --lib grad_fns::arithmetic`
  passes (the `tests` mod at `arithmetic.rs:4730-6523` covers forward,
...
- [x] AC-18: Non-contiguous CUDA views (transpose / narrow / permute) flow
  ...
  `ensure_contig_for_gpu` at `arithmetic.rs:112-157` (#812 cluster).
...
`ensure_contig_for_gpu` (`arithmetic.rs:112-157`) is the #812 fix-up that
...
(`arithmetic.rs:164-174`) check `is_grad_enabled()` + per-tensor
`requires_grad()`. `reduce_grad_to_shape` (`arithmetic.rs:880`) is the
...
`(reduce(grad, a.shape()), reduce(grad, b.shape()))` (`arithmetic.rs:1100-1105`).
The forward `pub fn add` (`arithmetic.rs:1094`) emits a profiler scope,
checks device-match, lets `meta_propagate::binary_broadcast` short-circuit
for meta tensors, then calls `add_inner`. `add_inner` (`arithmetic.rs:1116`)
...
`AddScaledBackward` (`arithmetic.rs:1221`) saves `a`, `b`, and `alpha:
f64`; backward returns `(reduce(grad, a.shape()), reduce(alpha*grad,
b.shape()))`. `scale_tensor` (`arithmetic.rs:1293`) is a private helper
that routes to dtype-specialised GPU `scale_*` kernels (f32/f64/bf16/f16)
or to `scalar_map` on CPU. `check_out_allowed` (`arithmetic.rs:1437`)
enforces torch's `out=` rules: no grad_fn, no requires_grad-leaf. `add_out`
(`arithmetic.rs:1495`) is an `alpha=1.0` wrapper over `add_scaled_out`.
`add_scaled_out` (`arithmetic.rs:1545`) validates devices, computes
...
proof. `add_scaled` (`arithmetic.rs:1622`) shortcircuits `alpha == 1.0`
...
- **Fused single-launch GPU path (#1675)**: `maybe_add_scaled_fused_gpu`
  (`arithmetic.rs:1738`) returns `Some(result)` when `a` and `b` are both
...
**Non-test consumers**: `inplace.rs:520` — `Tensor::add_scaled_` invokes
`arithmetic::add_scaled` and swaps the result storage in-place to satisfy
torch's `tensor.add_(other, *, alpha=1)` API; `inplace.rs:500`
...
WGAN-GP penalty via `sub`; `inplace.rs:628` (`Tensor::sub_scaled_`)
...
`MulBackward` (`arithmetic.rs:1890`) saves `a`/`b`; backward has two
...
(`arithmetic.rs:1949`) is structurally identical to `add`/`add_inner`
...
`DivBackward` (`arithmetic.rs:2070`) — `da = grad / b`,
`db = -grad * a / (b*b)`. Forward `div`/`div_inner`
(`arithmetic.rs:2132`) — `dispatch_floating_dtype!` over
...
`NegBackward` (`arithmetic.rs:2255`) returns `-grad` under `no_grad`.
Forward `neg`/`neg_inner` (`arithmetic.rs:2284`) routes through
...
`PowBackward` (`arithmetic.rs:2335`) saves `a` + `exp: f64`; backward
...
`pow`/`pow_inner` (`arithmetic.rs:2455`) uses
...
`SqrtBackward` (`arithmetic.rs:2527`) — `grad / (2 * sqrt(a))`. GPU
...
direct zip-map. Forward `sqrt`/`sqrt_inner` (`arithmetic.rs:2596`)
...
`AbsBackward` (`arithmetic.rs:4582`) — `grad * sign(a)` with explicit
`sign(0) = 0` convention and dedicated GPU `abs_backward_*`
kernel path when both `grad_output` and `a` live on CUDA. Forward
`abs`/`abs_inner` (`arithmetic.rs:4668`) uses
`backend.abs_{f32,f64,bf16_bf16,f16}` on CUDA and `unary_map(a, |x| x.abs())`
on CPU. **Non-test consumer**:
`methods.rs:55-58` `Tensor::abs_t`.

### REQ-9 `rsub` (one-line delegation)

`rsub` (`arithmetic.rs:1875`) is a one-line wrapper over `sub_scaled(b, a,
...
`RemainderBackward` (`arithmetic.rs:3014`) saves `a`/`b`; backward
...
what brought the parity-sweep delta below the `rtol=1e-5, atol=1e-7`
  ...
  consumer**: `methods.rs:106` (`Tensor::remainder_t`). CUDA f32/f64/f16/bf16
  inputs stay resident through backend broadcast kernels; CPU inputs keep
  the reference host loop above.

### REQ-14 `fmod` (lines 2256-2462)

`FmodBackward` (`arithmetic.rs:3288`) saves `a`/`b`; backward
...
recovers Python `%` / NumPy divisor-sign semantics). **Non-test
  consumer**: `methods.rs:124` (`Tensor::fmod_t`). CUDA f32/f64/f16/bf16
  inputs stay resident through backend broadcast kernels; CPU inputs keep
  the reference host loop above.
...
Forward `pub fn floor_divide` (`arithmetic.rs:3706`) emits a
profiler scope, checks device match, lets
...
CUDA f32/f64/f16/bf16 inputs stay resident through backend broadcast
kernels for `rounding_mode="floor"`; CPU inputs keep the reference
`c10::div_floor_floating`-style host kernel.

### REQ-15 `addcmul`

`AddcmulBackward` (`arithmetic.rs:3913`) saves `input` / `tensor1`
...
Forward `pub fn addcmul` (`arithmetic.rs:4097`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
tensor2), and calls `addcmul_inner`. The inner (`arithmetic.rs:4017`)
performs a 3-way broadcast: chains `broadcast_shapes(t1, t2)` then
`broadcast_shapes(input, t12)` to derive the output shape. CUDA
f32/f64/f16/bf16 inputs route through `cuda_float_ternary_storage`
(`arithmetic.rs:331`), materialize any non-contiguous CUDA views with
device-side strided copy, and dispatch dtype-specific fused broadcast
ternary kernels without `data_vec()` or a host round trip. f16/bf16 use
PyTorch CUDA opmath parity: load reduced storage, compute the fused
formula in f32, then round back to the storage dtype. CPU tensors use
the same broadcast shape, map each output coord into per-operand flat
indices with size-1 broadcast collapsing, then apply the fused
`out_i = input_i + value * tensor1_i * tensor2_i` step per upstream
byte-for-byte (R-DEV-1) mirroring `PointwiseOps.cpp:17-31`
meta-function `TensorIteratorConfig` with 3 const inputs. **Non-test consumer**:
`methods.rs:225` (`Tensor::addcmul_t`) delegates to
...
`AddcdivBackward` (`arithmetic.rs:4247`) saves `input` / `tensor1`
...
Forward `pub fn addcdiv` (`arithmetic.rs:4452`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
tensor2), and calls `addcdiv_inner`. The inner (`arithmetic.rs:4350`)
performs a 3-way broadcast: chains `broadcast_shapes(t1, t2)` then
`broadcast_shapes(input, t12)` to derive the output shape. CUDA
f32/f64/f16/bf16 inputs route through `cuda_float_ternary_storage`
(`arithmetic.rs:331`), materialize any non-contiguous CUDA views with
...
indices with size-1 broadcast collapsing, then apply the fused
`out_i = input_i + value * tensor1_i / tensor2_i` step per upstream
byte-for-byte (R-DEV-1) mirroring `PointwiseOps.cpp:33-52`
meta-function `build_ternary_op(...)` + the `addcdiv_stub` dispatch at
`PointwiseOps.cpp:66-73`. IEEE-754 div-by-zero at `tensor2_i=0`
produces +/-Inf (or NaN if `tensor1_i=0` too), matching upstream. The
integer-dtype hard error at `PointwiseOps.cpp:38-50
TORCH_META_FUNC(addcdiv)` is unreachable for the `Tensor<T: Float>`
family. **Non-test consumer**:
`methods.rs:264` (`Tensor::addcdiv_t`) delegates to
...
### Unit tests (in-file `#[cfg(test)] mod tests` at `arithmetic.rs:4730-6523`)
...
full impl-and-consumer evidence chain (`arithmetic.rs:3706` + `:3550` +
