...
- REQ-9: `rsub(a, b, *, alpha=1)` — `torch.rsub(input, other, *, alpha=1)`
  ...
  `arithmetic::rsub` at `grad_fns/arithmetic.rs:993` (one-line wrapper
...
- REQ-10: `rsqrt(a)` — `torch.rsqrt(input)` is `1 / sqrt(input)`. Per
  ...
  `arithmetic::rsqrt` at `grad_fns/arithmetic.rs:1757` (dedicated forward +
...
- REQ-11: `reciprocal(a)` — `torch.reciprocal(input)` is `1 / input`. Per
  ...
  `arithmetic::reciprocal` at `grad_fns/arithmetic.rs:1905` (dedicated
...
- REQ-12: `floor_divide(a, b)` — `torch.floor_divide(input, other)` floors
  ...
  `grad_fns/arithmetic.rs:2742` (CPU kernel mirroring
...
- REQ-13: `remainder(a, b)` — `torch.remainder(input, other)` returns the
  ...
  SHIPPED via `arithmetic::remainder` at `grad_fns/arithmetic.rs:2105`
...
- REQ-14: `fmod(a, b)` — `torch.fmod(input, other)` returns the remainder
  ...
  SHIPPED via `arithmetic::fmod` at `grad_fns/arithmetic.rs:2412` (CPU
...
- REQ-15: `addcmul(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcmul` at `grad_fns/arithmetic.rs:3089`
...
- REQ-16: `addcdiv(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcdiv` at `grad_fns/arithmetic.rs:3404`
...
- [x] AC-9: `rsub` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::rsub` (`grad_fns/arithmetic.rs:993`) delegating to
...
- [x] AC-10: `rsqrt` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::rsqrt` (`grad_fns/arithmetic.rs:1757`) + the dedicated
...
- [x] AC-11: `reciprocal` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::reciprocal` (`grad_fns/arithmetic.rs:1905`) + the dedicated
...
- [x] AC-13: `remainder` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::remainder` (`grad_fns/arithmetic.rs:2105`) + the dedicated
...
- [x] AC-14: `fmod` parity-sweep at `--seeds 8` returns
  ...
  #1199 via `arithmetic::fmod` (`grad_fns/arithmetic.rs:2412`) +
...
- [x] AC-12: `floor_divide` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::floor_divide` (`grad_fns/arithmetic.rs:2742`) +
...
- [x] AC-15: `addcmul` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::addcmul` (`grad_fns/arithmetic.rs:3089`) + the dedicated
...
- [x] AC-16: `addcdiv` parity-sweep at `--seeds 8` returns
  ...
  #1201 via `arithmetic::addcdiv` (`grad_fns/arithmetic.rs:3404`) +
...
- [x] AC-17: `cargo test -p ferrotorch-core --lib grad_fns::arithmetic`
  passes (the `tests` mod at `arithmetic.rs:3650-3993` covers forward,
...
proof. `add_scaled` (`arithmetic.rs:746-903`) shortcircuits `alpha == 1.0`
...
- **Fused single-launch GPU path (#1675)**: `maybe_add_scaled_fused_gpu`
  (`arithmetic.rs:849`) returns `Some(result)` when `a` and `b` are both
...
- `pub fn sub<T: Float>(a, b)` at `arithmetic.rs:912` is a one-line
  delegation: `sub_scaled(a, b, 1.0)`.
- `pub fn sub_scaled<T: Float>(a, b, alpha: f64)` at `arithmetic.rs:941`
...
`MulBackward` (`arithmetic.rs:1008-1062`) saves `a`/`b`; backward has two
...
(`arithmetic.rs:1067-1223`) is structurally identical to `add`/`add_inner`
...
`DivBackward` (`arithmetic.rs:1183-1216`) — `da = grad / b`,
`db = -grad * a / (b*b)`. Forward `div`/`div_inner`
(`arithmetic.rs:1227-1385`) — `dispatch_floating_dtype!` over
...
`NegBackward` (`arithmetic.rs:1345-1365`) returns `-grad` under `no_grad`.
Forward `neg`/`neg_inner` (`arithmetic.rs:1369-1460`) routes through
...
`PowBackward` (`arithmetic.rs:1420-1496`) saves `a` + `exp: f64`; backward
...
`pow`/`pow_inner` (`arithmetic.rs:1499-1587`) uses
...
`SqrtBackward` (`arithmetic.rs:1547-1594`) — `grad / (2 * sqrt(a))`. GPU
...
direct zip-map. Forward `sqrt`/`sqrt_inner` (`arithmetic.rs:1601-1665`)
...
`AbsBackward` (`arithmetic.rs:3537-3601`) — `grad * sign(a)` with explicit
...
`abs`/`abs_inner` (`arithmetic.rs:3608-3649`) uses
...
`rsub` (`arithmetic.rs:993`) is a one-line wrapper over `sub_scaled(b, a,
...
`RemainderBackward` (`arithmetic.rs:1991-2033`) saves `a`/`b`; backward
...
Forward `pub fn remainder` (`arithmetic.rs:2105`) emits a profiler
...
`FmodBackward` (`arithmetic.rs:2294-2336`) saves `a`/`b`; backward
...
treatment). Forward `pub fn fmod` (`arithmetic.rs:2412`) emits a
...
`FloorDivideBackward` (`arithmetic.rs:2585-2621`) saves `a`/`b`;
...
Forward `pub fn floor_divide` (`arithmetic.rs:2742-2760`) emits a
profiler scope, checks device match, lets
...
`AddcmulBackward` (`arithmetic.rs:2946-3043`) saves `input` / `tensor1`
...
Forward `pub fn addcmul` (`arithmetic.rs:3089-3115`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
tensor2), and calls `addcmul_inner`. The inner (`arithmetic.rs:3116-
...
`AddcdivBackward` (`arithmetic.rs:3242-3340`) saves `input` / `tensor1`
...
Forward `pub fn addcdiv` (`arithmetic.rs:3404-3429`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
tensor2), and calls `addcdiv_inner`. The inner (`arithmetic.rs:3431-
...
### Unit tests (in-file `#[cfg(test)] mod tests` at `arithmetic.rs:3650-5421`)
...
full impl-and-consumer evidence chain (`arithmetic.rs:2742` + `:2484` +
