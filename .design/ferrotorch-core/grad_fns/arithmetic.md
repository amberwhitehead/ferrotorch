...
- REQ-9: `rsub(a, b, *, alpha=1)` — `torch.rsub(input, other, *, alpha=1)`
  ...
  `arithmetic::rsub` at `grad_fns/arithmetic.rs:1001` (one-line wrapper
...
- REQ-10: `rsqrt(a)` — `torch.rsqrt(input)` is `1 / sqrt(input)`. Per
  ...
  `arithmetic::rsqrt` at `grad_fns/arithmetic.rs:1765` (dedicated forward +
...
- REQ-11: `reciprocal(a)` — `torch.reciprocal(input)` is `1 / input`. Per
  ...
  `arithmetic::reciprocal` at `grad_fns/arithmetic.rs:1913` (dedicated
...
- REQ-12: `floor_divide(a, b)` — `torch.floor_divide(input, other)` floors
  ...
  `grad_fns/arithmetic.rs:2750` (CPU kernel mirroring
...
- REQ-13: `remainder(a, b)` — `torch.remainder(input, other)` returns the
  ...
  SHIPPED via `arithmetic::remainder` at `grad_fns/arithmetic.rs:2113`
...
- REQ-14: `fmod(a, b)` — `torch.fmod(input, other)` returns the remainder
  ...
  SHIPPED via `arithmetic::fmod` at `grad_fns/arithmetic.rs:2420` (CPU
...
- REQ-15: `addcmul(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcmul` at `grad_fns/arithmetic.rs:3097`
...
- REQ-16: `addcdiv(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcdiv` at `grad_fns/arithmetic.rs:3412`
...
- [x] AC-9: `rsub` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::rsub` (`grad_fns/arithmetic.rs:1001`) delegating to
...
- [x] AC-10: `rsqrt` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::rsqrt` (`grad_fns/arithmetic.rs:1765`) + the dedicated
...
- [x] AC-11: `reciprocal` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::reciprocal` (`grad_fns/arithmetic.rs:1913`) + the dedicated
...
- [x] AC-13: `remainder` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::remainder` (`grad_fns/arithmetic.rs:2113`) + the dedicated
...
- [x] AC-14: `fmod` parity-sweep at `--seeds 8` returns
  ...
  #1199 via `arithmetic::fmod` (`grad_fns/arithmetic.rs:2420`) +
...
- [x] AC-12: `floor_divide` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::floor_divide` (`grad_fns/arithmetic.rs:2750`) +
...
- [x] AC-15: `addcmul` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::addcmul` (`grad_fns/arithmetic.rs:3097`) + the dedicated
...
- [x] AC-16: `addcdiv` parity-sweep at `--seeds 8` returns
  ...
  #1201 via `arithmetic::addcdiv` (`grad_fns/arithmetic.rs:3412`) +
...
- [x] AC-17: `cargo test -p ferrotorch-core --lib grad_fns::arithmetic`
  passes (the `tests` mod at `arithmetic.rs:3658-4001` covers forward,
...
`add_scaled_out` (`arithmetic.rs:685-748`) validates devices, computes
...
proof. `add_scaled` (`arithmetic.rs:754-911`) shortcircuits `alpha == 1.0`
...
- **Fused single-launch GPU path (#1675)**: `maybe_add_scaled_fused_gpu`
  (`arithmetic.rs:857`) returns `Some(result)` when `a` and `b` are both
...
- `pub fn sub<T: Float>(a, b)` at `arithmetic.rs:920` is a one-line
  delegation: `sub_scaled(a, b, 1.0)`.
- `pub fn sub_scaled<T: Float>(a, b, alpha: f64)` at `arithmetic.rs:949`
...
`MulBackward` (`arithmetic.rs:1016-1070`) saves `a`/`b`; backward has two
...
(`arithmetic.rs:1075-1231`) is structurally identical to `add`/`add_inner`
...
`DivBackward` (`arithmetic.rs:1191-1224`) — `da = grad / b`,
`db = -grad * a / (b*b)`. Forward `div`/`div_inner`
(`arithmetic.rs:1235-1393`) — `dispatch_floating_dtype!` over
...
`NegBackward` (`arithmetic.rs:1353-1373`) returns `-grad` under `no_grad`.
Forward `neg`/`neg_inner` (`arithmetic.rs:1377-1468`) routes through
...
`PowBackward` (`arithmetic.rs:1428-1504`) saves `a` + `exp: f64`; backward
...
`pow`/`pow_inner` (`arithmetic.rs:1507-1595`) uses
...
`SqrtBackward` (`arithmetic.rs:1555-1602`) — `grad / (2 * sqrt(a))`. GPU
...
direct zip-map. Forward `sqrt`/`sqrt_inner` (`arithmetic.rs:1609-1673`)
...
`AbsBackward` (`arithmetic.rs:3545-3609`) — `grad * sign(a)` with explicit
...
`abs`/`abs_inner` (`arithmetic.rs:3616-3657`) uses
...
`rsub` (`arithmetic.rs:1001`) is a one-line wrapper over `sub_scaled(b, a,
...
`RemainderBackward` (`arithmetic.rs:1999-2041`) saves `a`/`b`; backward
...
Forward `pub fn remainder` (`arithmetic.rs:2113`) emits a profiler
...
`FmodBackward` (`arithmetic.rs:2302-2344`) saves `a`/`b`; backward
...
treatment). Forward `pub fn fmod` (`arithmetic.rs:2420`) emits a
...
`FloorDivideBackward` (`arithmetic.rs:2593-2629`) saves `a`/`b`;
...
Forward `pub fn floor_divide` (`arithmetic.rs:2750-2768`) emits a
profiler scope, checks device match, lets
...
`AddcmulBackward` (`arithmetic.rs:2954-3051`) saves `input` / `tensor1`
...
Forward `pub fn addcmul` (`arithmetic.rs:3097-3123`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
tensor2), and calls `addcmul_inner`. The inner (`arithmetic.rs:3124-
...
`AddcdivBackward` (`arithmetic.rs:3250-3348`) saves `input` / `tensor1`
...
Forward `pub fn addcdiv` (`arithmetic.rs:3412-3437`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
tensor2), and calls `addcdiv_inner`. The inner (`arithmetic.rs:3439-
...
### Unit tests (in-file `#[cfg(test)] mod tests` at `arithmetic.rs:3658-5429`)
...
full impl-and-consumer evidence chain (`arithmetic.rs:2750` + `:2484` +
