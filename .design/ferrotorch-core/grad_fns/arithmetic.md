...
- REQ-9: `rsub(a, b, *, alpha=1)` — `torch.rsub(input, other, *, alpha=1)`
  ...
  `arithmetic::rsub` at `grad_fns/arithmetic.rs:867` (one-line wrapper
  ...
  `tools/parity-sweep/runner/src/main.rs:297`.
...
- REQ-10: `rsqrt(a)` — `torch.rsqrt(input)` is `1 / sqrt(input)`. Per
  ...
  `arithmetic::rsqrt` at `grad_fns/arithmetic.rs:1631` (dedicated forward +
  ...
  `tools/parity-sweep/runner/src/main.rs:320`.
...
- REQ-11: `reciprocal(a)` — `torch.reciprocal(input)` is `1 / input`. Per
  ...
  `tools/parity-sweep/runner/src/main.rs:328`.
...
- REQ-12: `floor_divide(a, b)` — `torch.floor_divide(input, other)` floors
  ...
  `grad_fns/arithmetic.rs:2616` (CPU kernel mirroring
  ...
  `tools/parity-sweep/runner/src/main.rs:374` (binary, no kwargs —
...
- REQ-13: `remainder(a, b)` — `torch.remainder(input, other)` returns the
  ...
  SHIPPED via `arithmetic::remainder` at `grad_fns/arithmetic.rs:1979`
  ...
  `tools/parity-sweep/runner/src/main.rs:341` (binary, no kwargs —
...
- REQ-14: `fmod(a, b)` — `torch.fmod(input, other)` returns the remainder
  ...
  SHIPPED via `arithmetic::fmod` at `grad_fns/arithmetic.rs:2286` (CPU
  ...
  #1199). Runner arm at `tools/parity-sweep/runner/src/main.rs:355`
...
- REQ-15: `addcmul(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcmul` at `grad_fns/arithmetic.rs:2963`
  ...
  * (tensor1 * value).conj())` (for `T: Float` real-only the
  ...
  #1200). Runner arm at `tools/parity-sweep/runner/src/main.rs:390`
...
- REQ-16: `addcdiv(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcdiv` at `grad_fns/arithmetic.rs:3278`
  ...
  #1201). Runner arm at `tools/parity-sweep/runner/src/main.rs:410`
...
- [x] AC-8: `pow` parity-sweep at `--seeds 8` returns `[pow] 24/72 passed
  ...
  `tools/parity-sweep/runner/src/main.rs:427` via blocker #1193.
- [x] AC-9: `rsub` parity-sweep at `--seeds 8` returns
  `[rsub] N/N passed (0 skipped, 0 failed)` with N >= 1. Closed by #1194
  via `arithmetic::rsub` (`grad_fns/arithmetic.rs:867`) delegating to
  `sub_scaled(b, a, alpha)` + the runner `"rsub"` arm at
  `tools/parity-sweep/runner/src/main.rs:297`. The forward result matches
...
- [x] AC-10: `rsqrt` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::rsqrt` (`grad_fns/arithmetic.rs:1631`) + dedicated
...
- [x] AC-13: `remainder` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::remainder` (`grad_fns/arithmetic.rs:1979`) + dedicated
  ...
  runner arm at `tools/parity-sweep/runner/src/main.rs:341`.
- [x] AC-14: `fmod` parity-sweep at `--seeds 8` returns
  `[fmod] N/N passed (0 skipped, 0 failed)` with N >= 1. Closed by
  #1199 via `arithmetic::fmod` (`grad_fns/arithmetic.rs:2286`) +
...
- [x] AC-15: `addcmul` parity-sweep at `--seeds 8` returns
  ...
  * tensor2`, `d_tensor2 = grad * value * tensor1` per
  ...
  `tools/parity-sweep/runner/src/main.rs:390` with new reusable
...
- [x] AC-16: `addcdiv` parity-sweep at `--seeds 8` returns
  ...
  #1201 via `arithmetic::addcdiv` (`grad_fns/arithmetic.rs:3278`) +
  ...
  `tools/parity-sweep/runner/src/main.rs:410` (reuses existing
...
(`arithmetic.rs:1101-1259`) — `dispatch_floating_dtype!` over
...
`pow`/`pow_inner` (`arithmetic.rs:1373-1461`) uses
...
`tools/parity-sweep/runner/src/main.rs:427`. op_db emits `pow` samples with
...
direct zip-map. Forward `sqrt`/`sqrt_inner` (`arithmetic.rs:1475-1565`)
...
`abs`/`abs_inner` (`arithmetic.rs:3482-3518`) uses
...
`rsub` (`arithmetic.rs:867`) is a one-line wrapper over `sub_scaled(b, a,
...
`tools/parity-sweep/runner/src/main.rs:297` (binary + alpha_kwarg,
mirrors the `"sub"` arm shape).

### REQ-13 `remainder` (lines 1865-2104)

`RemainderBackward` (`arithmetic.rs:1865-1907`) saves `a`/`b`; backward
...
Forward `pub fn remainder` (`arithmetic.rs:1979`) emits a profiler
...
`FmodBackward` (`arithmetic.rs:2168-2210`) saves `a`/`b`; backward
...
treatment). Forward `pub fn fmod` (`arithmetic.rs:2286`) emits a
...
`FloorDivideBackward` (`arithmetic.rs:2459-2495`) saves `a`/`b`;
...
Forward `pub fn floor_divide` (`arithmetic.rs:2616-2634`) emits a
profiler scope, checks device match, lets
...
`AddcmulBackward` (`arithmetic.rs:2820-2917`) saves `input` / `tensor1`
...
Forward `pub fn addcmul` (`arithmetic.rs:2963-2989`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
tensor2), and calls `addcmul_inner`. The inner (`arithmetic.rs:2991-
3083`) performs a 3-way broadcast: chains `broadcast_shapes(t1, t2)`
...
`tools/parity-sweep/runner/src/main.rs:390` introduces a new reusable
...
`AddcdivBackward` (`arithmetic.rs:3116-3214`) saves `input` / `tensor1`
...
Forward `pub fn addcdiv` (`arithmetic.rs:3278-3303`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
tensor2), and calls `addcdiv_inner`. The inner (`arithmetic.rs:3305-
3403`) performs a 3-way broadcast: chains `broadcast_shapes(t1, t2)`
then `broadcast_shapes(input, t12)` to derive the output shape, walks
the broadcast iteration order over the result, maps each output coord
into per-operand flat indices with size-1 broadcast collapsing, then
applies the fused `out_i = input_i + value * tensor1_i / tensor2_i`
...
style surface). The parity-sweep runner arm at
`tools/parity-sweep/runner/src/main.rs:410` REUSES the existing
...
full impl-and-consumer evidence chain (`arithmetic.rs:2616` + `:2459` +
