...
- REQ-10: `rsqrt(a)` — `torch.rsqrt(input)` is `1 / sqrt(input)`. Per
  ...
  `arithmetic::rsqrt` at `grad_fns/arithmetic.rs:2500` (dedicated forward +
...
- REQ-11: `reciprocal(a)` — `torch.reciprocal(input)` is `1 / input`. Per
  ...
  `arithmetic::reciprocal` at `grad_fns/arithmetic.rs:2652` (dedicated
...
- REQ-12: `floor_divide(a, b)` — `torch.floor_divide(input, other)` floors
  ...
  `grad_fns/arithmetic.rs:3509` (CPU kernel and CUDA broadcast kernels
...
- REQ-13: `remainder(a, b)` — `torch.remainder(input, other)` returns the
  ...
  SHIPPED via `arithmetic::remainder` at `grad_fns/arithmetic.rs:2853`
...
- REQ-14: `fmod(a, b)` — `torch.fmod(input, other)` returns the remainder
  ...
  SHIPPED via `arithmetic::fmod` at `grad_fns/arithmetic.rs:3167`
  (CPU kernel plus CUDA f32/f64/f16/bf16 broadcast kernels matching
...
- REQ-15: `addcmul(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcmul` at `grad_fns/arithmetic.rs:3931`
...
- REQ-16: `addcdiv(input, tensor1, tensor2, *, value=1)` — fused
  ...
  SHIPPED via `arithmetic::addcdiv` at `grad_fns/arithmetic.rs:4299`
...
- [x] AC-9: `rsub` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::rsub` (`grad_fns/arithmetic.rs:1630`) delegating to
...
- [x] AC-10: `rsqrt` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::rsqrt` (`grad_fns/arithmetic.rs:2500`) + the dedicated
...
- [x] AC-11: `reciprocal` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::reciprocal` (`grad_fns/arithmetic.rs:2652`) + the dedicated
...
- [x] AC-13: `remainder` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::remainder` (`grad_fns/arithmetic.rs:2853`) + the dedicated
...
- [x] AC-14: `fmod` parity-sweep at `--seeds 8` returns
  ...
  #1199 via `arithmetic::fmod` (`grad_fns/arithmetic.rs:3167`) +
...
- [x] AC-12: `floor_divide` parity-sweep at `--seeds 8` returns
  ...
  via `arithmetic::floor_divide` (`grad_fns/arithmetic.rs:3509`) +
...
- [x] AC-15: `addcmul` parity-sweep at `--seeds 8` returns
  ...
  `arithmetic::addcmul` (`grad_fns/arithmetic.rs:3931`) + the dedicated
...
- [x] AC-16: `addcdiv` parity-sweep at `--seeds 8` returns
  ...
  #1201 via `arithmetic::addcdiv` (`grad_fns/arithmetic.rs:4299`) +
...
The forward `pub fn add` (`arithmetic.rs:928`) emits a profiler scope,
...
`AddScaledBackward` (`arithmetic.rs:1055`) saves `a`, `b`, and `alpha:
f64`; backward returns `(reduce(grad, a.shape()), reduce(alpha*grad,
b.shape()))`. `scale_tensor` (`arithmetic.rs:1123`) is a private helper
that routes to dtype-specialised GPU `scale_*` kernels (f32/f64/bf16/f16)
or to `scalar_map` on CPU. `check_out_allowed` (`arithmetic.rs:1192`)
enforces torch's `out=` rules: no grad_fn, no requires_grad-leaf. `add_out`
(`arithmetic.rs:1250`) is an `alpha=1.0` wrapper over `add_scaled_out`.
`add_scaled_out` (`arithmetic.rs:1300`) validates devices, computes
...
proof. `add_scaled` (`arithmetic.rs:1377`) shortcircuits `alpha == 1.0`
...
- **Fused single-launch GPU path (#1675)**: `maybe_add_scaled_fused_gpu`
  (`arithmetic.rs:1486`) returns `Some(result)` when `a` and `b` are both
...
`MulBackward` (`arithmetic.rs:1645`) saves `a`/`b`; backward has two
...
(`arithmetic.rs:1704`) is structurally identical to `add`/`add_inner`
...
`DivBackward` (`arithmetic.rs:1825`) — `da = grad / b`,
`db = -grad * a / (b*b)`. Forward `div`/`div_inner`
(`arithmetic.rs:1887`) — `dispatch_floating_dtype!` over
...
`NegBackward` (`arithmetic.rs:2010`) returns `-grad` under `no_grad`.
Forward `neg`/`neg_inner` (`arithmetic.rs:2034`) routes through
...
`PowBackward` (`arithmetic.rs:2085`) saves `a` + `exp: f64`; backward
...
`pow`/`pow_inner` (`arithmetic.rs:2205`) uses
...
`SqrtBackward` (`arithmetic.rs:2271`) — `grad / (2 * sqrt(a))`. GPU
...
direct zip-map. Forward `sqrt`/`sqrt_inner` (`arithmetic.rs:2332`)
...
`rsub` (`arithmetic.rs:1630`) is a one-line wrapper over `sub_scaled(b, a,
...
`RemainderBackward` (`arithmetic.rs:2740`) saves `a`/`b`; backward
...
`FmodBackward` (`arithmetic.rs:3055`) saves `a`/`b`; backward
...
Forward `pub fn floor_divide` (`arithmetic.rs:3509`) emits a
profiler scope, checks device match, lets
...
`AddcmulBackward` (`arithmetic.rs:3753`) saves `input` / `tensor1`
...
Forward `pub fn addcmul` (`arithmetic.rs:3931`) emits a profiler
...
`AddcdivBackward` (`arithmetic.rs:4102`) saves `input` / `tensor1`
...
Forward `pub fn addcdiv` (`arithmetic.rs:4299`) emits a profiler
scope, checks pairwise device match (input vs tensor1, input vs
...
full impl-and-consumer evidence chain (`arithmetic.rs:3509` + `:3353` +
