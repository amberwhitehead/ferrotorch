...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (view) | SHIPPED | impl: `view_t` at `methods.rs:1064` (delegates to `crate::grad_fns::shape::reshape` at `methods.rs:1075`) mirrors upstream `TensorShape.cpp:4563 Tensor view`; non-test consumer: `Tensor::view` at `methods.rs:555`; lib test `test_resolve_shape_*` covers the shared `-1`-infer path; runner-arm gap #1340 (S5 test-infra). |
| REQ-2 (reshape) | SHIPPED | impl: `pub fn reshape` at `shape.rs:104` + `ReshapeBackward` at `shape.rs:69` mirrors upstream `TensorShape.cpp:2129 Tensor reshape`; non-test consumers: `methods.rs:501 reshape_t`, `flex_attention.rs:167/168/169/188/250/256`, `einsum.rs:1072/1081/1105/1107`; lib tests `test_reshape_forward/_backward/_infer_dim/_shape_mismatch/_no_grad`; runner-arm gap #1340. |
| REQ-3 (flatten) | SHIPPED | impl: `pub fn flatten` at `shape.rs:157` + `FlattenBackward` at `shape.rs:126` mirrors upstream `TensorShape.cpp:4178 Tensor flatten`; non-test consumers: `methods.rs:505 flatten_t`, `tensor.rs:1912` (FlattenBackward used in Tensor::flatten method body); lib tests `test_flatten_forward/_backward/_preserves_grad_fn`; runner-arm gap #1340. |
