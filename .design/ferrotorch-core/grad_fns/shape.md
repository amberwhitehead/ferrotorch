...
- REQ-24: `narrow(input, dim, start, length)` — `torch.narrow`. Per
  ...
  forward pub fn `narrow_t` lives at `narrow_t in methods.rs`.
...
- [x] AC-9: `narrow_t` (`narrow_t in methods.rs`) returns a zero-copy view
...
`narrow_t` (`narrow_t in methods.rs`) returns a zero-copy view with
