...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (sum) | NOT-STARTED | impl shipped: `pub fn sum` + `struct SumBackward` in `ferrotorch-core/src/grad_fns/reduction.rs` mirroring `aten/src/ATen/native/ReduceOps.cpp:1271 Tensor sum(...)` and `tools/autograd/derivatives.yaml:1702-1710 self: grad.expand_symint(self.sym_sizes())`. Non-test production consumers: `Tensor::sum_all` in `methods.rs` (boundary method, grandfathered per S5) + `crate::grad_fns::reduction::sum` invoked from `ferrotorch-core/src/autograd/grad_penalty.rs`, `einsum.rs`, `vmap.rs`, `flex_attention.rs`, `meta_propagate.rs` + `ferrotorch-nn/src/se.rs`, `ferrotorch-nn/src/linear.rs`, `ferrotorch-vision/src/models/yolo.rs`, `ferrotorch-nn/src/activation.rs`. Unit tests pass (`cargo test -p ferrotorch-core grad_fns::reduction::tests::test_sum_*`). **Blocked from SHIPPED on parity-sweep runner-arm gap #1314** — `./target/release/parity-sweep sweep --op sum --seeds 8` returns `0/80 passed (80 skipped, 0 failed)` because the runner has no `"sum" =>` dispatch arm. R-DEFER-6 requires parity smoke >= 1 for SHIPPED; the smoke count is 0. |
| REQ-2 (mean) | NOT-STARTED | impl shipped: `pub fn mean` + `struct MeanBackward` in `reduction.rs` mirroring `ReduceOps.cpp:1456 Tensor mean(...)` and `derivatives.yaml:1143-1151 self: grad.expand_symint(self.sym_sizes()) / self.sym_numel()`. Non-test production consumers: `Tensor::mean_all` in `methods.rs` (boundary method, grandfathered per S5) + transitively via the `reduction::mean` symbol. Unit tests pass. **Blocked on #1314** — `sweep --op mean` returns `0/80 passed (80 skipped, 0 failed)`. |
| REQ-3 (prod) | NOT-STARTED | impl shipped: `pub fn prod` + `struct ProdBackward` in `reduction.rs` mirroring `ReduceOps.cpp:1379 Tensor prod(...)` and `derivatives.yaml:1413-1415 self: prod_backward(...)` (prefix-suffix product VJP). Zero handling: single-zero distributes through, multi-zero is all-zero gradient, both verified by `test_prod_backward_with_zero` / `test_prod_backward_two_zeros`. Non-test production consumer: `Tensor::prod_all` in `methods.rs` (boundary method, grandfathered per S5). Unit tests pass including `test_prod_numerical_gradient`. **Blocked on #1314** — `sweep --op prod` returns `0/156 passed (156 skipped, 0 failed)`. |
| REQ-4 (amin/amax) | NOT-STARTED | impl shipped: `pub fn amin` / `pub fn amax` + `struct AminBackward` / `struct AmaxBackward` in `reduction.rs` mirroring `ReduceOps.cpp:1758 TORCH_IMPL_FUNC(amin_out)` / `:1766 TORCH_IMPL_FUNC(amax_out)` and `derivatives.yaml:1205-1211 self: scale_grad_by_count(...)`. Non-test production consumers: `Tensor::amin(&self)` / `Tensor::amax(&self)` in `methods.rs` (boundary methods, grandfathered per S5). NaN handling diverges from upstream (ferrotorch skips NaN, upstream poisons) — pinned under #1314. Dim-keyed variant `amin(self, dim, keepdim)` is NOT-STARTED (covered by #1302 alongside max/min with dim). **Blocked on #1314 for both ops** — `sweep --op amin` and `sweep --op amax` both return `0/80 passed (80 skipped, 0 failed)`. |
| REQ-5 (sum_dim) | NOT-STARTED | impl shipped: `pub fn sum_dim` + `struct SumDimBackward` in `reduction.rs` mirroring `ReduceOps.cpp:1245 TORCH_IMPL_FUNC(sum_out)` (single-dim variant of the multi-dim list `int[1]?` signature) and `derivatives.yaml:1712-1719 self: sum_backward(grad, self.sym_sizes(), dim, keepdim)`. Single-dim restriction (vs upstream `int[1]?` list) and 0-D rejection (vs upstream pass-through) are documented divergences. Non-test production consumers: `Tensor::sum_dim(&self, dim, keepdim)` in `methods.rs` (boundary method) + extensive callers in `einsum.rs`, `einops.rs`, `grad_fns/linalg.rs`, `ferrotorch-nn/src/functional.rs`, `ferrotorch-distributions/src/multivariate_normal.rs`, `dirichlet.rs`. Unit tests `test_sum_dim_*` cover 1D/2D/3D forward, negative dim, keepdim, backward. Re-exported at `ferrotorch-core/src/lib.rs`. **Blocked on #1314** — no parity-sweep `"sum_dim"` runner arm OR the dim-list form (the route lists `sum`, not `sum_dim`). |
| REQ-6 (mean_dim) | NOT-STARTED | impl shipped: `pub fn mean_dim` + `struct MeanDimBackward` in `reduction.rs` mirroring `ReduceOps.cpp:1396 TORCH_IMPL_FUNC(mean_out)` and `derivatives.yaml:1153-1155 self: mean_backward(grad, sizes, dim, numel, keepdim)`. Same single-dim and 0-D-reject divergences as sum_dim. Non-test production consumer: `Tensor::mean_dim(&self, dim, keepdim)` in `methods.rs` (boundary method) + `meta_propagate.rs` import + `ferrotorch-nn/src/functional.rs` import (`red::mean_dim` used inside the cosine-similarity functional family). Re-exported at `ferrotorch-core/src/lib.rs`. Unit tests `test_mean_dim_*` pass. **Blocked on #1314**. |
| REQ-7 (backward VJP wiring) | NOT-STARTED | impl shipped: all 7 `*Backward` structs (`SumBackward`, `MeanBackward`, `ProdBackward`, `AminBackward`, `AmaxBackward`, `SumDimBackward`, `MeanDimBackward`) in `reduction.rs` implement `crate::tensor::GradFn<T>` with `backward`, `inputs`, `name`. The attach decision (`is_grad_enabled() && input.requires_grad()`) is uniform across all 7. The `name()` strings match upstream PyTorch's grad-fn-name reporting (`SumBackward`, `MeanBackward`, `ProdBackward`, `AminBackward`, `AmaxBackward`, `SumDimBackward`, `MeanDimBackward`). Production consumers: every forward `pub fn` in this file constructs the corresponding `*Backward` via `Arc::new(...)` and attaches via `Tensor::from_operation`. Downstream autograd consumers — `Tensor::backward`, `autograd::grad`, `autograd::higher_order::*`, `autograd::checkpoint::*`, `autograd::grad_penalty`, `autograd::fixed_point`, `autograd::gradcheck`, `ops::higher_order::*` — walk these grad_fns in production code paths. Tests `test_*_has_grad_fn_*`, `test_*_no_grad_fn_*`, `test_*_numerical_gradient`, `test_*_backward_*` all pass. **Blocked from SHIPPED on #1314 the same way REQ-1..REQ-6 are** — without a parity-sweep runner arm exercising each forward, the smoke gate cannot fire on the autograd backward either. |
| REQ-8 (std/var) | NOT-STARTED | not implemented in `reduction.rs`. Upstream entry points `Tensor std(const Tensor& self, bool unbiased)` at `aten/src/ATen/native/ReduceOps.cpp:2105-2109` and `Tensor var(...)` at `:2085-2089` route through `std_var_stub` declared at `:449` and use Welford's online algorithm in `aten/src/ATen/native/SharedReduceOps.h:86-148 WelfordOps`. Open prereq blocker: #1301. |
...
| REQ-10 (argmax/argmin) | NOT-STARTED | not implemented in `reduction.rs`. Upstream `TORCH_IMPL_FUNC(argmax_out)` at `aten/src/ATen/native/ReduceOps.cpp:1809-1815` / `TORCH_IMPL_FUNC(argmin_out)` at `:1817-1823`. Integer-output, non-differentiable. Requires ferrotorch's integer-tensor handling for the output (which exists for `gather` / `scatter` indices but not as a primary reduction output). Open prereq blocker: #1304. |
...
| REQ-13 (logsumexp autograd) | NOT-STARTED | kernel-layer forward exists at `ferrotorch-core/src/ops/elementwise.rs:1233 pub fn logsumexp` (and `:1269 pub fn logsumexp_dim`) but no autograd wrapper. Upstream `Tensor logsumexp(const Tensor& self, IntArrayRef dims, bool keepdim)` at `aten/src/ATen/native/ReduceOps.cpp:1548-1559`. VJP per `derivatives.yaml` is `grad * exp(input - result)` (softmax-weighted routing). Open prereq blocker: #1310. |
| REQ-14 (any/all/count_nonzero) | NOT-STARTED | not implemented in `reduction.rs`. Upstream `TORCH_IMPL_FUNC(all_out)` at `aten/src/ATen/native/ReduceOps.cpp:1667-1670` / `TORCH_IMPL_FUNC(any_out)` at `:1681-1684`; `count_nonzero` lives in `aten/src/ATen/native/SummaryOps.cpp`. Non-differentiable (no `derivatives.yaml` entry). Open prereq blocker: #1312. |
| REQ-15 (parity-sweep runner arms) | NOT-STARTED | no runner arms in `tools/parity-sweep/runner/src/main.rs` for any of `sum`, `mean`, `prod`, `amin`, `amax`, `std`, `var`, `max`, `min`, `argmax`, `argmin`, `median`, `nanmedian`, `norm`, `logsumexp`, `any`, `all`, `count_nonzero`. The 5 cumulative ops (`cumsum`, `cumprod`, `cummax`, `cummin`, `logcumsumexp`) DO have runner arms at `runner/src/main.rs:608-680` and pass parity, but they are owned by `grad_fns/cumulative.rs` (not this file). Open prereq blocker: #1314. |
