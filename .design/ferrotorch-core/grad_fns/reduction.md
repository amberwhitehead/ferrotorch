...
- REQ-1: `sum(input)` — forward `out = sum_i input[i]` collapsing to a
  ...
  (test-driver loss — but `nn/src/linear.rs` are non-test
...
- REQ-13: `logsumexp(input)` / `logsumexp(input, dim, keepdim)` —
  ...
  ` pub fn logsumexp_dim`, but **no autograd wrapper** is present
...
- REQ-15: Parity-sweep runner arms — every op the route's
  ...
  `logcumsumexp in runner/src/main.rs`), which are owned by
...
| `max` / `min` (with dim) | `Tensor max(...) -> std::tuple<Tensor, Tensor>` family at `ReduceOps.cpp` | `value_selecting_reduction_backward` per `:2372` | SHIPPED — `pub fn max_with_dim` / `pub fn min_with_dim` in `reduction.rs` return `(Tensor<T>, IntTensor<i64>)`; shared `MaxMinDimBackward` scatters grad at saved per-slice argmax/argmin index. NaN-poisoning per `SharedReduceOps.h:26-34`. Consumer: `ferrotorch_core::{max_with_dim, min_with_dim}` re-exports at `lib.rs:182`. Closes #1302. |
...
| `norm` | `ReduceOps.cpp:1590 TORCH_IMPL_FUNC(norm_out)` → `norm_stub` declared at `:451` | `norm_backward` via `SharedReduceOps.h:247 NormOps` / `:285 NormZeroOps` / `:315 NormOneOps` | SHIPPED for `p > 0 finite` — `pub fn norm_with_dim(input, p, dim, keepdim)` + `NormDimBackward` in `reduction.rs` implements `(sum(|x|^p, dim))^(1/p)` with the closed-form VJP `|x|^(p-1) * sign(x) * result^(1-p) * grad` and `result==0 → 0` mask. `p=0` / `p=inf` / negative `p` rejected at the API boundary (piecewise / non-diff cases tracked separately). FORWARD F32 L2 PRECISION (#1614): for `p == 2.0`, `T == f32`, last-dim-contiguous slices (`inner == 1`), `norm_with_dim` reduces via `simd_reduce::l2_norm_f32_torch` (the torch vectorized last-dim L2 kernel model, `ReduceOpsKernel.cpp:222-255`, f32 accumulator) instead of the generic f64 `Σ|x|^p` then `^(1/p)`, so the f32 norm value matches torch byte-for-byte (modulo a documented ~3% one-ULP residual). The `p != 2` / non-f32 / non-last-dim cases keep the f64 generic path. Consumer: `ferrotorch_core::norm_with_dim` re-export at `lib.rs:182`. Closes #1308; satisfies #1614 for this file's `norm` path. |
...
| REQ-5 (sum_dim) | SHIPPED | impl: `pub fn sum_dim` + `struct SumDimBackward` in `reduction.rs` mirroring `ReduceOps.cpp:1245 TORCH_IMPL_FUNC(sum_out)` and `derivatives.yaml:1712-1719 self: sum_backward(grad, self.sym_sizes(), dim, keepdim)`. Single-dim restriction; 0-D rejection documented in AC-16. Non-test consumers: `Tensor::sum_dim` + `einsum.rs`, `einops.rs`, `grad_fns/linalg.rs`, `ferrotorch-nn/src/functional.rs`, `ferrotorch-distributions/...`. Re-exported in `lib.rs`. Runner exercises this via the `"sum"` arm's dim-chain path; parity sweep `--seeds 8` returns `[sum] 160/160 passed (0 skipped, 0 failed)` (smoke=1). FORWARD CPU PERF (#1689): the contiguous CPU forward now decomposes the reduced dim as `[outer, axis, inner]` and runs the AVX-friendly fast accumulate in `fn reduce_axis_sum_contiguous` instead of the per-element odometer scan — `inner > 1` uses a contiguous lane-add (torch's `vectorized_outer_reduction`, `Reduce.h:94-113`), `inner == 1` (last-dim) routes f32/f64 rows through `simd_reduce::sum_f32` / `sum_f64` (torch's `vectorized_inner_reduction`, `Reduce.h:80-91`). `[1000,1000]` dim=0 dropped ~10,118us → ~78us. Non-contiguous inputs are materialised contiguous first (transposed correctness pinned by `divergence_axis_reduction_strided_reaudit`). |
| REQ-6 (mean_dim) | SHIPPED | impl: `pub fn mean_dim` + `struct MeanDimBackward` in `reduction.rs` mirroring `ReduceOps.cpp:1396 TORCH_IMPL_FUNC(mean_out)` and `derivatives.yaml:1153-1155 self: mean_backward(grad, sizes, dim, numel, keepdim)`. Same single-dim and 0-D-reject divergences as sum_dim. Non-test consumers: `Tensor::mean_dim` + `meta_propagate.rs` + `ferrotorch-nn/src/functional.rs`. Re-exported in `lib.rs`. Runner exercises via `"mean"` arm's dim-chain path; parity sweep `[mean] 160/160 passed (0 skipped, 0 failed)` (smoke=1). FORWARD CPU PERF (#1689): `mean_dim` forward = `reduce_axis_sum_contiguous` (the same `[outer, axis, inner]` fast accumulate sum_dim uses) then divide by `axis`; `[1000,1000]` dim=1 (the `inner == 1` SIMD horizontal-sum regime) dropped ~10,480us → ~190us. Correctness across the rank × dim × keepdim × {f32,f64} matrix (incl. the 3-D middle-dim probe) pinned by `divergence_axis_reduction_simd_fastpath_1689`. |
