...
- [ ] AC-10: All 35 `parity_ops` from the route table return `N/N passed
  ...
  **fused-affine + shape subset** of #1344 gained runner arms 2026-05-27
  ...
  forwards per REQ-5..9/29..35). As of 2026-05-28 the **FINAL linalg
  subset** of #1344 gained runner arms (`solve` / `qr` / `cholesky` / `inv`
  / `det` / `slogdet` / `cross`, REQ-10/16/17/18/20/21/25), compared unique
  / gauge-invariantly against custom well-conditioned samples in
  `oracle.py`'s `_CUSTOM_OPS`: `solve 40/40`, `qr 40/40` (reconstruction
  `Q@R≈A`), `cholesky 40/40` (`L` direct + `L@Lᵀ≈A`), `inv 40/40`,
  `det 40/40`, `slogdet 40/40` (concat `[sign, logabsdet]`), `cross 32/32`,
  all **0 skipped, 0 failed** at seeds=8. The same commit removed the
  obsolete `addr` `{beta:0, alpha:0, self=NaN}` runner skip (the #1598
  forward bug is fixed in `grad_fns/linalg.rs` commit `1a43ddeee`), so
  `addr` now reaches `48/48 passed (0 skipped, 0 failed)`. The LAST
  outstanding linalg runner arm — `vector_norm` (REQ-23, distinct from
  `matrix_norm`/`norm`: different `ord` semantics) — was wired 2026-05-28
  (#1599): the `"vector_norm"` arm of `dispatch_f32` routes through
  `crate::linalg::vector_norm`, compared DIRECTLY (unique scalar) against a
  custom `_CUSTOM_OPS["vector_norm"]` adapter in `oracle.py` sweeping the
  op_db `ord` vocabulary (`1`/`2`/`3`/`4`/`6`/`0.9`/`+inf`/`-inf`) over
  full-tensor (`dim=None`) non-zero samples, reaching **`384/384 passed
  (0 skipped, 0 failed)`** at seeds=8. With these wired, EVERY op in this
  file that has a runner-arm-able forward is now covered; `eig`/`eigvals`
  BACKWARD stays NOT-STARTED under #1345 (their FORWARDs already have runner
  arms). The linalg runner-arm scope of #1344 is now COMPLETE.
...
Two residual legitimate-skip pathways (each documented in the runner arm +
`parity_audit.json`): (1) `diagonal` skips op_db's 3-D `[5,5,5]`
non-default-`dim1`/`dim2` samples (the forward is the 2-D `dim1=0,dim2=1`
contract, REQ-30); (2) `tril` / `triu` skip op_db's batched `[5,10,5]` /
`[3,3,5,5]` inputs (the forwards are the strict 2-D contract, REQ-32/33;
batched triangular is a residual under #1344). The `addr` `{beta:0,
alpha:0, self=NaN}` skip that previously masked the #1598 forward bug
(torch's `beta==0` `addr_kernel` at
`aten/src/ATen/native/cpu/LinearAlgebraKernel.cpp:53-55` DROPS the `self`
term so NaN/inf do not propagate) was REMOVED 2026-05-28: the forward bug
is fixed in `grad_fns/linalg.rs` commit `1a43ddeee` (#1598 — the
fused-affine forwards now branch on `beta == 0` and never read `self`), so
the sample RUNS and `addr` reaches `48/48 passed (0 skipped, 0 failed)`.

The **DECOMPOSITION subset** of #1344 gained runner arms 2026-05-27
(svd / eig / eigh / eigvals / eigvalsh / pinv / lstsq / lu / lu_factor /
householder_product / norm / matrix_rank), each compared
**GAUGE-INVARIANTLY** because eigenvectors / singular vectors carry
sign / phase / ordering freedom (comparing raw `U` / `V` / `Q` element-wise
against torch's LAPACK gauge would spuriously FAIL even when the math is
correct). The runner arm computes — on the ferrotorch side — the SAME
gauge-invariant derived quantity the oracle's `_<op>_torch_call` adapter
emits (`tools/parity-sweep/oracle.py` `_CUSTOM_OPS`), so the harness's
`assert_close_f32` value gate compares them apples-to-apples:

| op | derived quantity (gauge-invariant comparison) |
|---|---|
| `svd` | singular VALUES `S` (gauge-free) **+** reconstruction `U diag(S) Vh ≈ A` |
| `eigh` | eigenVALUES `w` (real, ascending, gauge-free) **+** reconstruction `Q diag(w) Q^T ≈ B` |
| `eigvalsh` | eigenVALUES `w` (real, ascending) — direct |
| `eig` | complex eigenvalue SET sorted by `(re, im)`; runner arm also asserts its OWN residual `‖A v − λ v‖ < 1e-3·‖A‖_F` locally (a wrong forward Errs, not skips) |
| `eigvals` | complex eigenvalue SET sorted by `(re, im)` — direct |
| `pinv` | Moore-Penrose pseudo-inverse (UNIQUE) — direct |
| `lstsq` | full-rank least-squares solution (UNIQUE) — direct |
| `lu` | reconstruction `P L U ≈ A` |
| `lu_factor` | reconstruction `P L U ≈ A` rebuilt from packed `(LU, pivots)` |
| `householder_product` | `Q` (DETERMINISTIC fn of `(input, tau)` from `torch.geqrf`) — direct |
| `norm` | Frobenius / 2-norm SCALAR — direct |
| `matrix_rank` | numerical rank INTEGER — direct |

The samples are hand-crafted well-conditioned real f32 matrices
(square / tall / wide / SPD / rank-deficient where the op needs it) in the
`_CUSTOM_OPS` registry, NOT op_db's raw-tuple samples (op_db hands back the
`(U, S, Vh)` / `(w, Q)` / `(P, L, U)` tuples which the harness's
`output[0]`-selecting gate cannot compare gauge-invariantly). The
ferrotorch-side derived quantities are assembled by the `ferrotorch_svd_probe`
/ `ferrotorch_eigh_probe` / `ferrotorch_eig_probe` / `ferrotorch_lu_probe` /
`ferrotorch_lu_factor_probe` helpers (and inline arms for the unique-output
ops) in `tools/parity-sweep/runner/src/main.rs`. Verified 2026-05-27 with
`parity-sweep sweep --op <op> --seeds 8`, all **0 failed**:

```
[svd]                  64/64  passed (0 skipped, 0 failed)   smoke grep count = 1
[eig]                  40/40  passed (0 skipped, 0 failed)   smoke grep count = 1
[eigh]                 48/48  passed (0 skipped, 0 failed)   smoke grep count = 1
[eigvals]              40/40  passed (0 skipped, 0 failed)   smoke grep count = 1
[eigvalsh]             48/48  passed (0 skipped, 0 failed)   smoke grep count = 1
[pinv]                 64/64  passed (0 skipped, 0 failed)   smoke grep count = 1
[lstsq]                40/40  passed (0 skipped, 0 failed)   smoke grep count = 1
[lu]                   64/64  passed (0 skipped, 0 failed)   smoke grep count = 1
[lu_factor]            40/40  passed (0 skipped, 0 failed)   smoke grep count = 1
[householder_product]  40/40  passed (0 skipped, 0 failed)   smoke grep count = 1
[norm]                 48/48  passed (0 skipped, 0 failed)   smoke grep count = 1
[matrix_rank]          48/48  passed (0 skipped, 0 failed)   smoke grep count = 1
```

The decomposition family uses the widened `rtol=1e-4` (atol=1e-6) envelope
in `fn tolerance_for` for the same faer-vs-LAPACK factorization f32 ULP drift
the matmul / det / inv family documents (amplified for the reconstruction
probes by the extra `diag`-scale + GEMM round). Per goal.md S5: missing
runner arms were a TEST-INFRASTRUCTURE gap, not a REQ blocker — these ops
were SHIPPED via their grad-aware production forwards (REQ-11..28) regardless
of the runner-arm wiring state; this wiring closes the decomposition subset of
the test-infrastructure umbrella #1344. (`eig`/`eigvals` BACKWARD remain
NOT-STARTED under #1345 — the complex-autograd blocker; the sweep exercises
their forward only.)

The **FINAL linalg subset** of #1344 gained runner arms 2026-05-28
(`solve` / `qr` / `cholesky` / `inv` / `det` / `slogdet` / `cross`), each
fed custom well-conditioned f32 samples in `oracle.py`'s `_CUSTOM_OPS`
(keyed by the bare op names the runner's `dispatch_f32` matches) and
compared UNIQUE / GAUGE-INVARIANTLY:

| op | derived quantity (comparison) |
|---|---|
| `solve` | the solution `X` of `A X = B` (UNIQUE for invertible `A`) — direct |
| `qr` | reconstruction `Q @ R ≈ A` (raw Q/R column-sign freedom cancels; torch fixes signs by R's diagonal, faer uses its own convention) |
| `cholesky` | `L` (UNIQUE for SPD) — direct **+** reconstruction `L @ Lᵀ ≈ A` |
| `inv` | the inverse `A^{-1}` (UNIQUE) — direct |
| `det` | the determinant SCALAR (UNIQUE, 0-D `[]`) — direct |
| `slogdet` | `[sign, logabsdet]` (sign EXACT in `{-1,+1}`, logabsdet a real scalar) — direct |
| `cross` | the vector cross product (UNIQUE bilinear) — direct |

The `qr` samples are `m ≥ n` only (the `m < n` backward is a residual under
#1577); the `cholesky` samples are SPD (`G Gᵀ + n·I`, matching torch's
op_db cholesky contract); the `slogdet` samples include a negative-
determinant (odd-permutation) case so `sign` exercises both `+1` and `-1`.
The ferrotorch side calls the grad-aware production forwards
`crate::linalg::{solve,qr,cholesky,inv,det,slogdet,cross}` (the
`torch.linalg.*` public surface, REQ-10/16/17/18/20/21/25), with the
`qr` / `cholesky` reconstructions assembled by `ferrotorch_qr_probe` /
`ferrotorch_cholesky_probe` in `tools/parity-sweep/runner/src/main.rs`.
Verified 2026-05-28 with `parity-sweep sweep --op <op> --seeds 8`, all
**0 failed**:

```
[solve]    40/40 passed (0 skipped, 0 failed)   smoke grep count = 1
[qr]       40/40 passed (0 skipped, 0 failed)   smoke grep count = 1
[cholesky] 40/40 passed (0 skipped, 0 failed)   smoke grep count = 1
[inv]      40/40 passed (0 skipped, 0 failed)   smoke grep count = 1
[det]      40/40 passed (0 skipped, 0 failed)   smoke grep count = 1
[slogdet]  40/40 passed (0 skipped, 0 failed)   smoke grep count = 1
[cross]    32/32 passed (0 skipped, 0 failed)   smoke grep count = 1
```

The same dispatch also REMOVED the obsolete `addr` `{beta:0, alpha:0,
self=NaN}` runner skip (the #1598 forward bug is fixed in
`grad_fns/linalg.rs` commit `1a43ddeee`), so `addr` now reaches
`48/48 passed (0 skipped, 0 failed)`. The FINAL subset uses the same
widened `rtol=1e-4` (atol=1e-6) decomposition envelope in `fn tolerance_for`
(`solve`/`inv`/`det`/`slogdet` for the LU drift, `qr`/`cholesky` for the QR/
Cholesky drift + reconstruction GEMM round; `cross` is exact but rides the
envelope for uniformity). Per goal.md S5 the runner arms were a TEST-
INFRASTRUCTURE gap, not a REQ blocker — these ops were already SHIPPED via
their grad-aware production forwards (REQ-10/16/17/18/20/21/25); this wiring
closes the FINAL subset of the umbrella #1344.
