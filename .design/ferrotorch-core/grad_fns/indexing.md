...
| REQ-4 (scatter_reduce) | SHIPPED | impl: `pub fn scatter_reduce` + `pub enum ScatterReduce` + `pub struct ScatterReduceBackward` in `grad_fns/indexing.rs` mirroring `aten/src/ATen/native/TensorAdvancedIndexing.cpp:2354 TORCH_IMPL_FUNC(scatter_reduce_two)`. Forward supports reduce ∈ {sum, prod, amax, amin} with `include_self ∈ {true, false}` (mean is out of scope — separate work item). Forward attaches `ScatterReduceBackward` for ALL reduce modes per upstream `derivatives.yaml:3074-3077` (live oracle confirms torch sets `r.grad_fn = <ScatterReduceBackward0 ...>` for every mode). Backward implements ALL four modes per `torch/csrc/autograd/FunctionsManual.cpp:7194-7279` `scatter_reduce_backward`: sum gathers grad at index positions; amax/amin distribute grad uniformly to positions whose value matched the result; prod uses the masked-self-result / masked-self chain-rule with zero-handling (single-zero path uses scatter of `masked_src_result`, all-non-zero uses `(grad * result).gather / src`). Saves the forward `result` buffer on the GradFn for the value-aware VJPs. **Runner arm** in `tools/parity-sweep/runner/src/main.rs` decoding `[input_f32, dim_i64, index_int64, src_f32, reduce_str]` + `include_self` kwarg. **Non-test production consumer**: `pub fn scatter_reduce_t` on `Tensor` in `methods.rs` (the chainable method-style surface, mirroring `torch.Tensor.scatter_reduce`) delegating to `grad_fns::indexing::scatter_reduce`. Parity gate: **`[scatter_reduce] 144/168 passed (24 skipped, 0 failed)` at seeds 0..8** — 0 failures, 86% pass; skips are 0-d input / ndim-mismatch (narrower contract). Closes #1245 / #1288. |
