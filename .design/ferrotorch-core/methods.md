...
- [x] AC-1: Every arithmetic method (`add_t / sub_t / mul_t / div_t / neg_t /
  ...
  `test_method_chain` at `ferrotorch-core/src/methods.rs:835-840` and the
  matmul / relu / sum / sigmoid tests at `:805-846` plus the conformance
...
- [x] AC-2: Reshape / squeeze / unsqueeze / permute / view / transpose are
  ...
  forward pass) — covered by `test_method_permute_2d` at
  `ferrotorch-core/src/methods.rs:884-894` which asserts `!b.is_contiguous()`
  after permute, and by `test_method_view` at `:923-929`.
- [x] AC-3: `permute` rejects invalid permutations (wrong length, duplicate,
  out-of-bounds) with `InvalidArgument` — covered by
  `test_permute_invalid_dims` at `ferrotorch-core/src/methods.rs:913-919`.
- [x] AC-4: `chunk(n, dim)` produces `n` chunks (or fewer when the last is
  ...
  split — covered by `test_method_chunk_even / _uneven / _2d` at
  `ferrotorch-core/src/methods.rs:950-977`.
- [x] AC-5: `split(split_sizes, dim)` rejects mismatched-sum split_sizes —
  covered by `test_split_bad_sizes` at `ferrotorch-core/src/methods.rs:1001
  -1005`.
- [x] AC-6: `split` / `chunk` preserve autograd (each output chunk has a
  ...
  `test_split_preserves_grad / test_split_backward_simple /
  test_chunk_backward_2d / test_split_no_grad_when_disabled` at
  `ferrotorch-core/src/methods.rs:1009-1112`.
- [x] AC-7: `contiguous()` on an already-contiguous tensor returns a cheap
  ...
  R-CODE-4) — covered by `test_method_contiguous` at
  `ferrotorch-core/src/methods.rs:940-946` and exercised in production at
...
- [x] AC-8: `view(&[..])` requires `is_contiguous()` and errors with
  ...
  inference for exactly one dim — covered by `test_method_view /
  test_method_view_infer` at `ferrotorch-core/src/methods.rs:923-936`.
- [x] AC-9: `narrow(dim, start, length)` errors when `start + length >
  ...
  the sliced dimension (`NarrowBackward` at
  `ferrotorch-core/src/methods.rs:440-493`).
...
- [x] AC-11: `size() == shape()` and `dim() == ndim()` byte-for-byte —
  covered by `test_size_alias / test_dim_alias` at
  `ferrotorch-core/src/methods.rs:1117-1128`.
...
`ferrotorch-diffusion/src/blocks.rs:337,340 (bmm)`,
...
`ferrotorch-core/src/flex_attention.rs:212,213,226 (squeeze_t / unsqueeze_t)`,
...
- `view(&[i64])` → free function `view_t` (`methods.rs:499-511`) which
  validates contiguity and forwards to `grad_fns::shape::reshape`.
- `contiguous()` → free function `contiguous_t` (`methods.rs:524-587`) with
  ...
  (`ContiguousBackward` at `:591-611`).
- `narrow(dim, start, length)` → free function `narrow_t` (`methods.rs:393
  -437`) — zero-copy view; `NarrowBackward` at `:441-493` pads the gradient
...
- `chunk(chunks, dim)` → `chunk_t` (`methods.rs:617-652`) which
  computes per-chunk sizes (`dim_size.div_ceil(chunks)`) and delegates to
  `split_t`.
- `split(split_sizes, dim)` → `split_t` (`methods.rs:659-786`). GPU fast
...
- The zero-copy stride-view construction (`permute_t` at `:311-357`,
  `narrow_t` at `:393-437`).
- The autograd `*Backward` structs (`PermuteBackward` at `:361-387`,
  `NarrowBackward` at `:441-493`, `ContiguousBackward` at `:591-611`).
- The GPU fast paths (`contiguous_t` uses `backend.strided_copy_{f32,f64}`
  for rank ≤ 8 CUDA inputs at `:541-548`; `split_t` uses
  `backend.strided_split_f32` per-chunk at `:712-738`).
- The CPU fallbacks (`contiguous_t_cpu` at `:571-587` always-valid layout
  copy; `split_t`'s outer-strides loop at `:752-783`).
...
- **Arithmetic/activation chain**: `test_method_relu` (`:796`),
  `test_method_matmul` (`:805`), `test_method_sum` (`:817`),
  `test_method_transpose` (`:824`), `test_method_chain` (`:835`),
  `test_method_sigmoid` (`:843`), `test_method_flatten` (`:850`).
- **Reduction**: `test_method_sum_dim` (`:866`), `test_method_mean_dim`
  (`:875`).
- **Shape**: `test_method_permute_2d` (`:884`), `test_method_permute_3d`
  (`:902`), `test_permute_invalid_dims` (`:914`),
  `test_method_view` (`:924`), `test_method_view_infer` (`:932`),
  `test_method_contiguous` (`:941`).
- **Chunk/split**: `test_method_chunk_even` (`:951`),
  `test_method_chunk_uneven` (`:961`), `test_method_chunk_2d` (`:971`),
  `test_method_split` (`:982`), `test_method_split_2d_axis1` (`:991`),
  `test_split_bad_sizes` (`:1002`).
- **Autograd**: `test_split_preserves_grad` (`:1010`),
  `test_split_backward_simple` (`:1024`), `test_chunk_backward_2d`
  (`:1069`), `test_split_no_grad_when_disabled` (`:1102`).
- **Aliases**: `test_size_alias` (`:1117`), `test_dim_alias` (`:1124`).
- **Utility**: `test_method_print_chain` (`:857`).
...
| REQ-6 (linalg methods) | SHIPPED | impl: `Tensor::matmul / mm / mm_bt / bmm / mv_t / dot_t / t / einsum` at `ferrotorch-core/src/methods.rs:141-187` delegate to `crate::grad_fns::linalg::*_differentiable` and `crate::einsum::einsum_differentiable` mirroring `torch.Tensor.matmul / mm / bmm / mv / dot / t` per `torch/_tensor_docs.py:6388 / 3394 / 1060 / 3475 / 1771 / 5201` and `torch.einsum` per `torch/functional.py:176`. Non-test consumers: `ferrotorch-distributions/src/multivariate_normal.rs:232,265,297,320 eps.matmul(&l_t) / scale_tril.matmul / diff_2d.matmul`; `ferrotorch-optim/src/muon.rs:216 g.t()`; `ferrotorch-optim/src/natural_gradient.rs:275,279 input_activation.t() / output_gradient.t()`; `ferrotorch-diffusion/src/attention.rs:190,197 q.bmm(&k_t) / probs.bmm(&v)`; `ferrotorch-diffusion/src/blocks.rs:337,340 q.bmm(&k_t) / probs.bmm(&v)`. Caveat: `einsum / mv_t / dot_t / mm_bt / mm` have no direct non-test caller (those callers prefer `matmul` or call the underlying `linalg::*_differentiable` directly); the family-level REQ is SHIPPED via `matmul / bmm / t`. |
...
| REQ-8 (reshape/shape methods) | SHIPPED | impl: `Tensor::reshape_t / flatten_t / squeeze_t / unsqueeze_t / permute / transpose` at `ferrotorch-core/src/methods.rs:201-241` (transpose at `:228-241` builds the permutation array and delegates to `permute_t`). Mirrors `torch.Tensor.reshape / flatten / squeeze / unsqueeze / permute / transpose` per `aten/src/ATen/native/TensorShape.cpp:2129 / 4178 / 4014 / 4109 / 1829 / 3816` and `torch/_tensor_docs.py:4274 / 2061 / 5019 / 6109 / 3777 / 5903`. Non-test consumers: `ferrotorch-nn/src/attention.rs:275,292,312,313,314,337,340,343,347,355-365,408,411,420,579 (reshape_t)`; `ferrotorch-diffusion/src/attention.rs:173-186,202-205,735-746 (reshape_t / transpose)`; `ferrotorch-diffusion/src/blocks.rs:309-317,348 (reshape_t / transpose)`; `ferrotorch-diffusion/src/resnet_block_time.rs:133 (reshape_t)`; `ferrotorch-nn/src/rnn.rs:244,245,669,1711 (squeeze_t via narrow)`; `ferrotorch-core/src/einops.rs:799,805 (squeeze_t)`; `ferrotorch-core/src/flex_attention.rs:212,213,226 (squeeze_t / unsqueeze_t)`; `ferrotorch-vision/src/models/vit.rs:437,570 (squeeze_t)`; `ferrotorch-ml/src/metrics.rs:1495,1496 (reshape_t)`; `ferrotorch-nn/src/attention.rs:338,344,348,369 (permute)`. |
| REQ-9 (view/contiguous/narrow) | SHIPPED | impl: `Tensor::view / contiguous / narrow` at `ferrotorch-core/src/methods.rs:247-263` delegate to free functions `view_t` (`:499-511`), `contiguous_t` (`:524-587` with GPU `strided_copy_{f32,f64}` fast path), `narrow_t` (`:393-437`). `ContiguousBackward` at `:591-611`, `NarrowBackward` at `:441-493`. Mirrors `aten/src/ATen/native/TensorShape.cpp:4563 view / 1669 narrow` and `torch/_tensor_docs.py:6145 / 1190 / 3502`. Non-test consumers: pervasive — `ferrotorch-distributions/src/multivariate_normal.rs:243,271,319,359,367 (view)`; `ferrotorch-distributions/src/dirichlet.rs:291,293,350,358,362 (view)`; `ferrotorch-vision/src/models/swin.rs:273,276,360,370 + vit.rs:100 (view)`; `ferrotorch-diffusion/src/attention.rs:175,180,185,204,737,745 + nn/src/attention.rs:339,345,349,410 (contiguous)`; `ferrotorch-nn/src/rnn.rs:244,245 (narrow)`; `ferrotorch-data/src/transforms.rs:465,466 (narrow)`; `ferrotorch-core/src/flex_attention.rs:210,211 (narrow)`; `ferrotorch-core/src/nested.rs:549,556 (narrow)`; `ferrotorch-core/src/einops.rs:798,804 (narrow)`. |
