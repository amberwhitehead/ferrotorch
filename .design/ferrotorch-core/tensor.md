...
| REQ-6 | SHIPPED | impl: `to_pinned(device)` at `ferrotorch-core/src/tensor.rs:1145`; non-test consumer: `ferrotorch-data::DataLoader` calls this when `pin_memory(true)` is set. |
| REQ-7 | SHIPPED | impl: `data` at `ferrotorch-core/src/tensor.rs:709`, `data_ref` at `:748`, `data_vec` at `:758`; non-test consumer: every CPU op that reads tensor data (e.g. `pruning::magnitude_prune` at `pruning.rs:269`, `signal::windows`'s round-trip helper). |
...
| REQ-9 | SHIPPED | impl: `detach` at `ferrotorch-core/src/tensor.rs:1862`, `requires_grad_` at `:1880`; non-test consumer: `autograd::no_grad` blocks call `detach`; downstream model init code calls `requires_grad_(true)` on parameters. |
...
| REQ-13 | SHIPPED | impl: `update_data` at `ferrotorch-core/src/tensor.rs:1368`, `update_storage` at `:1562`, `update_storage_and_shape` at `:1455`, `with_gpu_handle_mut` at `:1623`; non-test consumer: `ferrotorch-nn::optim::adamw` / `sgd` `step()` calls these to write the updated parameter; the `out=`-style add path at `grad_fns/arithmetic.rs::add_scaled_out` calls `update_storage_and_shape`. |
...
| REQ-15 | SHIPPED | impl: `masked_fill` at `ferrotorch-core/src/tensor.rs:1261`, `masked_select` at `:1277`; non-test consumer: production ops in `grad_fns/indexing.rs::masked_fill_bt` (called via the inherent method delegate). |
