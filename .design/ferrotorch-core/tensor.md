...
| REQ-9 | SHIPPED | impl: `detach` at `ferrotorch-core/src/tensor.rs:2481`, `requires_grad_` at `:2502`; non-test consumer: `autograd::no_grad` blocks call `detach`; downstream model init code calls `requires_grad_(true)` on parameters. |
