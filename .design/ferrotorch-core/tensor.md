...
| REQ-3 | SHIPPED | impl: `from_operation` at `ferrotorch-core/src/tensor.rs:470`; non-test consumer: every grad-attaching forward op (e.g. `grad_fns/arithmetic.rs:596` in `add_inner`). |
...
| REQ-7 | SHIPPED | impl: `data` at `ferrotorch-core/src/tensor.rs:1185`, `data_ref` at `:1235`, `data_vec` at `:1245`; non-test consumer: every CPU op that reads tensor data (e.g. `pruning::magnitude_prune` at `pruning.rs:72`, `signal::windows`'s round-trip helper). |
| REQ-8 | SHIPPED | impl: `grad` at `ferrotorch-core/src/tensor.rs:1020`, `set_grad` at `:1032` with shape/device validation, `zero_grad` at `:1051`, `accumulate_grad` at `:1063` with GPU-native path at `:1085-1112` and shared metadata guard at `:1148-1163`; non-test consumer: `autograd::backward` engine writes via `accumulate_grad` for every leaf reachable in the graph. |
| REQ-9 | SHIPPED | impl: `pub fn detach in ferrotorch-core/src/tensor.rs`, `pub fn requires_grad_ in ferrotorch-core/src/tensor.rs`; non-test consumer: `autograd::no_grad` blocks call `detach`; downstream model init code calls `requires_grad_(true)` on parameters. |
...
| REQ-12 | SHIPPED | impl: `gpu_handle` at `ferrotorch-core/src/tensor.rs:1850`; non-test consumer: every CUDA op that dispatches a kernel (e.g. `grad_fns/arithmetic.rs::add_inner` for the CUDA branch, `stride_tricks.rs:1110-1156`). |
| REQ-13 | SHIPPED | impl: `update_data` at `ferrotorch-core/src/tensor.rs:1984`, `update_storage` at `:2237`, `update_storage_and_shape` at `:2092`, `with_gpu_handle_mut` at `:2458`; non-test consumer: `ferrotorch-nn::optim::adamw` / `sgd` `step()` calls these to write the updated parameter; the `out=`-style add path at `grad_fns/arithmetic.rs::add_scaled_out` calls `update_storage_and_shape`. |
