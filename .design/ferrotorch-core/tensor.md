...
| REQ-7 | SHIPPED | impl: `data` at `ferrotorch-core/src/tensor.rs:709`, `data_ref` at `:748`, `data_vec` at `:758`; non-test consumer: every CPU op that reads tensor data (e.g. `pruning::magnitude_prune` at `pruning.rs:31`, `signal::windows`'s round-trip helper). |
