...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `Tensor<T>` struct at `ferrotorch-core/src/tensor.rs:101`, `TensorInner` at `:76`; non-test consumer: every op in the workspace constructs / consumes `Tensor<T>` — see e.g. `grad_fns/arithmetic.rs:376` (`add`'s forward). |
| REQ-2 | SHIPPED | impl: `from_storage` at `ferrotorch-core/src/tensor.rs:109`; non-test consumer: `creation::zeros` / `ones` / `tensor` at `creation.rs:10, 17, 24` and ~all op forward paths. |
...
| REQ-4 | SHIPPED | impl: `view_reshape` at `ferrotorch-core/src/tensor.rs:150`, `view_operation` at `:198`, `stride_view` at `:248`, `stride_view_operation` at `:272`; non-test consumer: `grad_fns/shape.rs` reshape / flatten / squeeze / unsqueeze ops, `methods.rs::view_t`. |
| REQ-5 | SHIPPED | impl: `to(device)` at `ferrotorch-core/src/tensor.rs:787-1002` covers CPU/CUDA/XPU/Meta matrix incl. the #802 non-contiguous CUDA→CPU materialise path at `:862-881`; non-test consumer: `Tensor::cuda` at `:1056`, `Tensor::cpu` at `:1061`, plus every model state-dict load that targets a device. |
| REQ-6 | SHIPPED | impl: `to_pinned(device)` at `ferrotorch-core/src/tensor.rs:1019`; non-test consumer: `ferrotorch-data::DataLoader` calls this when `pin_memory(true)` is set. |
| REQ-7 | SHIPPED | impl: `data` at `ferrotorch-core/src/tensor.rs:635`, `data_ref` at `:674`, `data_vec` at `:684`; non-test consumer: every CPU op that reads tensor data (e.g. `pruning::magnitude_prune` at `pruning.rs:31`, `signal::windows`'s round-trip helper). |
| REQ-8 | SHIPPED | impl: `grad` at `ferrotorch-core/src/tensor.rs:516`, `set_grad` at `:528`, `zero_grad` at `:544`, `accumulate_grad` at `:554` with GPU-native path at `:574-595`; non-test consumer: `autograd::backward` engine writes via `accumulate_grad` for every leaf reachable in the graph. |
| REQ-9 | SHIPPED | impl: `detach` at `ferrotorch-core/src/tensor.rs:1417`, `requires_grad_` at `:1435`; non-test consumer: `autograd::no_grad` blocks call `detach`; downstream model init code calls `requires_grad_(true)` on parameters. |
| REQ-10 | SHIPPED | impl: `is_contiguous` at `ferrotorch-core/src/tensor.rs:1457`, `is_contiguous_for` at `:1486`, `to_memory_format` at `:1516`, `contiguous_in` at `:1531`, `materialize_format` at `:1542` (GPU fast path at `:1575-1623`); non-test consumer: `ferrotorch-nn::Conv2d` calls `to_memory_format(MemoryFormat::ChannelsLast)` before cuDNN dispatch. |
...
| REQ-12 | SHIPPED | impl: `gpu_handle` at `ferrotorch-core/src/tensor.rs:1106`; non-test consumer: every CUDA op that dispatches a kernel (e.g. `grad_fns/arithmetic.rs::add_inner` for the CUDA branch, `stride_tricks.rs:394-417`). |
| REQ-13 | SHIPPED | impl: `update_data` at `ferrotorch-core/src/tensor.rs:1196`, `update_storage` at `:1325`, `update_storage_and_shape` at `:1269`, `with_gpu_handle_mut` at `:1391`; non-test consumer: `ferrotorch-nn::optim::adamw` / `sgd` `step()` calls these to write the updated parameter; the `out=`-style add path at `grad_fns/arithmetic.rs::add_scaled_out` calls `update_storage_and_shape`. |
| REQ-14 | SHIPPED | impl: `register_hook` at `ferrotorch-core/src/tensor.rs:460`, `register_post_accumulate_grad_hook` at `:480`, `remove_hook` at `:500`; non-test consumer: `autograd::hooks::HookStorage` integration; user-callable surface for gradient monitoring. |
| REQ-15 | SHIPPED | impl: `masked_fill` at `ferrotorch-core/src/tensor.rs:1126`, `masked_select` at `:1142`; non-test consumer: production ops in `grad_fns/indexing.rs::masked_fill_bt` (called via the inherent method delegate). |
| REQ-16 | SHIPPED | impl: `item` at `ferrotorch-core/src/tensor.rs:1699`; non-test consumer: every scalar-returning op result (loss values, mean, sum_all, etc.) — used pervasively by training-loop diagnostics. |
| REQ-17 | SHIPPED | impl: `into_storage_and_shape` at `ferrotorch-core/src/tensor.rs:736`; non-test consumer: `accumulate_grad` at `:565` uses it to take ownership of the incoming grad's storage without copying. |
| REQ-18 | SHIPPED | impl: `is_same` at `ferrotorch-core/src/tensor.rs:1713`, `inner_refcount` at `:1722`, `storage_refcount` at `:1732`, `inner_storage_arc` at `:1742`; non-test consumer: the autograd backward engine (`autograd::graph`) uses refcount checks to gate in-place gradient accumulation. |
