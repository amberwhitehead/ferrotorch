...
- `ferrotorch-core/src/ops/indexing.rs:1186, :1260, :1286` — kernel-layer
  ops that take `&BoolTensor` parameters (`where_cond_bt_strict`,
  `where_cond_bt`, `masked_select`).
...
| REQ-11 | SHIPPED | impl: constructors and logical/comparison ops return `FerrotorchResult` for `ShapeMismatch`, `DeviceMismatch`, `InvalidArgument`, `DeviceUnavailable`, and backend kernel errors; no public logical op panics to report ordinary backend failure. Non-test production consumer: the same `grad_fns/indexing.rs` and `grad_fns/comparison.rs` paths propagate structured errors via `?`. |
