...
- `ferrotorch-core/src/dtype_dispatch.rs:26 / :125-150` — `is_f32`,
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `pub use ferray_core::{DType, Element}` at `ferrotorch-core/src/dtype.rs:6` mirroring `c10::ScalarType` (`c10/core/ScalarType.h:38-43`). Non-test production consumer: `ferrotorch-core/src/lib.rs:142` re-exports `DType, Element, Float` for downstream crates, and `ferrotorch-core/src/storage.rs` uses `<T as Element>::dtype()` to tag every GPU buffer (the on-device dtype carrier). |
| REQ-2 | SHIPPED | impl: trait `Float` at `ferrotorch-core/src/dtype.rs:26` with bound `Element + num_traits::Float + std::ops::AddAssign`, implementors at `:28-35` (`f32`, `f64`, `bf16`, `f16`). Non-test production consumer: `ferrotorch-core/src/tensor.rs` `pub struct Tensor<T: Float>` and every `<T: Float>` op in `ferrotorch-core/src/grad_fns/arithmetic.rs` (e.g. `pub fn add<T: Float>` at `arithmetic.rs:108`). |
