...
- The `hanning(m)` re-export at `hanning in windows.rs` is `#[inline]` and
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 | SHIPPED | impl: `bartlett in ferrotorch-core/src/signal/windows.rs`, `blackman in ferrotorch-core/src/signal/windows.rs`, `hamming in ferrotorch-core/src/signal/windows.rs`, `hann in ferrotorch-core/src/signal/windows.rs`, `hanning in ferrotorch-core/src/signal/windows.rs`, `kaiser in ferrotorch-core/src/signal/windows.rs` mirror `torch/signal/windows/windows.py:{bartlett, blackman, hamming, hann, kaiser}` and `numpy.{bartlett, blackman, hamming, hanning, kaiser}`; non-test consumer: re-exported at `ferrotorch-core/src/signal/mod.rs`, reachable by FFT-based audio / DSP code (production user surface — STFT and spectrogram code applies windows to input frames). |
| REQ-2 | SHIPPED | impl: `cosine in ferrotorch-core/src/signal/windows.rs`, `exponential in ferrotorch-core/src/signal/windows.rs`, `gaussian in ferrotorch-core/src/signal/windows.rs`, `general_cosine in ferrotorch-core/src/signal/windows.rs`, `general_hamming in ferrotorch-core/src/signal/windows.rs`, `nuttall in ferrotorch-core/src/signal/windows.rs`, `parzen in ferrotorch-core/src/signal/windows.rs`, `taylor in ferrotorch-core/src/signal/windows.rs`, `tukey in ferrotorch-core/src/signal/windows.rs` mirror `torch.signal.windows.*`; non-test consumer: re-exported at `ferrotorch-core/src/signal/mod.rs`. |
...
| REQ-4 | SHIPPED | impl: each function propagates `ferray_window::Error::InvalidValue` via `.map_err(FerrotorchError::Ferray)?` (e.g. `signal/windows.rs`, `signal/windows.rs`, etc.); non-test consumer: the error path is hit by callers passing invalid params — production downstream code that constructs a window from a runtime parameter (e.g. an STFT block sized by a config file) receives the error rather than a panic. Test pin at `, 410-413, 511-514, 541-546`. |
| REQ-5 | SHIPPED | impl: `pub fn hanning` at `hanning in ferrotorch-core/src/signal/windows.rs` is `#[inline]` and body `hann(m)`; non-test consumer: re-exported at `signal/mod.rs` alongside `hann`. Test pin at `hann in signal/windows.rs` asserts bit-equality. |
