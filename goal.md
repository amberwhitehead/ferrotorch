...
- **S5 — R-DEFER-1 binds on NEWLY-ADDED pub APIs only.** Existing pub API surface across multiple prior commits is grandfathered. Boundary methods (`Tensor::add_t`) ARE the public API; they don't need further downstream callers to be SHIPPED.
- **S6 — Model tier per role.** Opus for acto-critic + novel algorithm work (backward, broadcasting, dtype promotion). Sonnet for mirror-pattern builders ("ship sub_scaled like add_scaled") and most fixers. Haiku for cite refreshes, fixture bumps, REQ-table line updates. Specify with `model:` field in the Agent dispatch.
