...
use ferrotorch_nn::{Embedding, Module, Parameter};
...
fn sparse_adam_trajectory() {
    ...
    for case in fix["sparse_adam"].as_array().unwrap() {
        ...
        // torch.optim.SparseAdam REQUIRES a sparse gradient (it raises on a
        // dense one — sparse_adam.py:88-92). We model the dense 10-element
        // grad as a fully-materialised sparse-COO grad over a `[10,1]` param:
        // every row index is present, so the masked update covers all
        // coordinates and reproduces the exact SparseAdam trajectory
        // (`_functional.py:24-84`). The fixture is generated by the same
        // masked `step_size = lr*sqrt(bc2)/bc1` formula
        // (`scripts/regenerate_optim_adam_fixtures.py:run_sparse_adam`),
        // validated byte-for-byte against live torch 2.11.0.
        let n = init.len();
        let data2d: Vec<f64> = init.clone();
        let param = Parameter::new(
            Tensor::from_storage(TensorStorage::cpu(data2d), vec![n, 1], true).unwrap(),
        );
        ...
        let mut results: Vec<Vec<f64>> = Vec::with_capacity(5);
        for grad in grads.iter().take(5) {
            // Sparse grad over all `n` rows (slab_size 1): indices 0..n, one
            // value per row.
            let sg = SparseGrad::<f64>::new((0..n).collect(), grad.clone(), vec![1]).unwrap();
            opt.set_sparse_grad(0, 0, sg);
            opt.step().unwrap();
            results.push(param.tensor().data_vec().unwrap());
        }
...
#[test]
fn sparse_adam_skips_untouched_rows() {
    // torch.optim.SparseAdam updates ONLY the rows present in the sparse
    // grad; rows absent from the COO indices keep their moments at zero and
    // their param byte-stable (`_functional.py:65-72`). A `[3,1]` param with
    // a sparse grad touching ONLY row 0 leaves rows 1 and 2 untouched.
    let param = Parameter::new(
        Tensor::from_storage(TensorStorage::cpu(vec![1.0, 2.0, 3.0]), vec![3, 1], true).unwrap(),
    );
    let mut opt = SparseAdam::new(vec![param.clone()], SparseAdamConfig::default());
    // Sparse grad: only index 0, value 0.1.
    let sg = SparseGrad::<f64>::new(vec![0], vec![0.1], vec![1]).unwrap();
    opt.set_sparse_grad(0, 0, sg);
    opt.step().unwrap();

    let v = param.tensor().data_vec().unwrap();
    // Index 0 should have changed.
    assert_ne!(v[0], 1.0, "SparseAdam: touched row should update");
    // Rows 1 and 2 absent from the COO grad -> byte-stable.
    assert_eq!(v[1], 2.0, "SparseAdam: untouched row should not update");
    assert_eq!(v[2], 3.0, "SparseAdam: untouched row should not update");
}

/// End-to-end production chain: `nn::Embedding(sparse=true)` forward →
/// weight grad → `SparseAdam::collect_sparse_grad_from_embedding` → `step`.
/// Mirrors the upstream `nn.Embedding(sparse=True)` → `torch.optim.SparseAdam`
/// flow (`torch/nn/modules/sparse.py:34,48`, `torch/optim/sparse_adam.py:132-161`).
///
/// Oracle (live torch.optim.SparseAdam 2.11.0): weight `[4,2]` pinned to
/// `i*10` row-major; sparse grad at rows {0,2} with slabs `[0,0]` and `[2,2]`;
/// lr=0.5, betas=(0.9,0.999), eps=1e-8. Result (flattened):
/// `[0, 10, 20, 30, 39.50000007905693, 49.50000007905693, 60, 70]`
/// (row 0 grad is zero -> no move; row 2 moves by the masked step).
#[test]
fn sparse_adam_consumes_embedding_sparse_grad_end_to_end() {
    let mut emb = Embedding::<f64>::new(4, 2, None).unwrap().with_sparse(true);
    let init: Vec<f64> = (0..4 * 2).map(|i| i as f64 * 10.0).collect();
    emb.weight =
        Parameter::new(Tensor::from_storage(TensorStorage::cpu(init), vec![4, 2], true).unwrap());

    // Forward over rows {0, 2} records the touched indices for sparse_grad.
    let idx = Tensor::from_storage(TensorStorage::cpu(vec![0.0f64, 2.0]), vec![2], false).unwrap();
    let _ = emb.forward(&idx).unwrap();

    // Synthetic weight grad: row r = [r, r]; only rows {0,2} are touched, so
    // sparse_grad pulls slabs r0=[0,0], r2=[2,2].
    let grad_vec: Vec<f64> = (0..4_usize)
        .flat_map(|r| vec![r as f64, r as f64])
        .collect();
    let grad_tensor =
        Tensor::from_storage(TensorStorage::cpu(grad_vec), vec![4, 2], false).unwrap();
    emb.weight.tensor().set_grad(Some(grad_tensor)).unwrap();

    // Build the optimizer over the embedding's weight parameter and wire the
    // sparse grad through the production helper.
    let mut opt = SparseAdam::new(
        vec![emb.weight.clone()],
        SparseAdamConfig::default()
            .with_lr(0.5)
            .with_betas((0.9, 0.999))
            .with_eps(1e-8),
    );
    let registered = opt.collect_sparse_grad_from_embedding(&emb, 0, 0).unwrap();
    assert!(registered, "embedding should yield a sparse grad");
    opt.step().unwrap();

    let got = opt.param_groups()[0].params()[0]
        .tensor()
        .data_vec()
        .unwrap();
    let expected = [
    ...
    for (i, (&g, &e)) in got.iter().zip(expected.iter()).enumerate() {
        assert!(
            (g - e).abs() <= 1e-11,
            "row-major[{i}]: expected {e:.17}, got {g:.17}"
        );
    }
}
