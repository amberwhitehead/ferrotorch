...
//! | REQ-5 (reductions) | SHIPPED | `pub fn gpu_sum_bf16 / gpu_mean_bf16 / gpu_prod_bf16 / gpu_sum_axis_bf16_bf16 / gpu_mean_axis_bf16_bf16 / gpu_prod_axis_bf16 / gpu_extreme_axis_bf16 / gpu_extreme_axis_backward_bf16 in bf16.rs`; consumer bf16 reduction arms in `backend_impl.rs` |
...
const CROSS_BF16_PTX: &str = "\
.version 7.0
.target sm_52
.address_size 64

.visible .entry cross_bf16_kernel(
    .param .u64 a_ptr,
    .param .u64 b_ptr,
    .param .u64 out_ptr,
    ...
    .reg .b16 %a0_b16, %a1_b16, %a2_b16, %b0_b16, %b1_b16, %b2_b16, %zero16;
    .reg .b32 %a0_u32, %a1_u32, %a2_u32, %b0_u32, %b1_u32, %b2_u32, %bits, %round, %lsb;
    ...
    shl.b64 %off, %off, 1;
    add.u64 %addr, %a, %off;
    ld.global.b16 %a0_b16, [%addr];
    add.u64 %addr, %b, %off;
    ld.global.b16 %b0_b16, [%addr];
    mov.b32 %a0_u32, {%zero16, %a0_b16};
    mov.b32 %b0_u32, {%zero16, %b0_b16};
    mov.b32 %a0, %a0_u32;
    mov.b32 %b0, %b0_u32;
    ...
    shl.b64 %off, %off, 1;
    add.u64 %addr, %a, %off;
    ld.global.b16 %a1_b16, [%addr];
    add.u64 %addr, %b, %off;
    ld.global.b16 %b1_b16, [%addr];
    mov.b32 %a1_u32, {%zero16, %a1_b16};
    mov.b32 %b1_u32, {%zero16, %b1_b16};
    mov.b32 %a1, %a1_u32;
    mov.b32 %b1, %b1_u32;
    ...
    shl.b64 %off, %off, 1;
    add.u64 %addr, %a, %off;
    ld.global.b16 %a2_b16, [%addr];
    add.u64 %addr, %b, %off;
    ld.global.b16 %b2_b16, [%addr];
    mov.b32 %a2_u32, {%zero16, %a2_b16};
    mov.b32 %b2_u32, {%zero16, %b2_b16};
    mov.b32 %a2, %a2_u32;
    mov.b32 %b2, %b2_u32;
...
const CLAMP_BF16_PTX: &str = "\
.version 7.0
.target sm_52
.address_size 64

.visible .entry clamp_bf16_kernel(
    .param .u64 in_ptr,
    .param .u64 out_ptr,
    .param .u32 n,
    .param .f32 min_val,
...
const CLAMP_BACKWARD_BF16_PTX: &str = "\
.version 7.0
.target sm_52
.address_size 64

.visible .entry clamp_backward_bf16_kernel(
    .param .u64 grad_ptr,
    .param .u64 input_ptr,
    .param .u64 out_ptr,
    .param .f32 min_val,
    ...
    .reg .u32 %r_tid, %bid, %bdim, %n_reg;
    .reg .u64 %g, %x, %out, %off;
    .reg .b16 %g_b16, %x_b16, %zero16;
    .reg .b32 %g_u32, %x_u32, %bits, %round, %lsb;
    ...
    ld.global.b16 %g_b16, [%g];
    ld.global.b16 %x_b16, [%x];
    mov.b16 %zero16, 0;
...
fn launch_clamp(
    ...
    let n_u32 = n as u32;
    // SAFETY:
    // - `f` is the `clamp_bf16_kernel` PTX entry with ABI
    //   `(in_ptr, out_ptr, n, min_val, max_val)`, matching the args below.
    // - `a` is a bf16 u16 buffer of length `n`; `out` is freshly allocated
...
fn launch_clamp_backward(
    ...
    let f = get_or_compile(
        ctx,
        CLAMP_BACKWARD_BF16_PTX,
        "clamp_backward_bf16_kernel",
        device.ordinal() as u32,
    )
    .map_err(|e| GpuError::PtxCompileFailed {
        kernel: "clamp_backward_bf16_kernel",
        source: e,
    })?;
    let mut out = stream.alloc_zeros::<u16>(n)?;
    let cfg = launch_1d(n);
    let n_u32 = n as u32;
    // SAFETY:
    // - `f` is the `clamp_backward_bf16_kernel` entry with ABI
...
/// Same-shape `torch.linalg.cross` on bf16 (u16-stored) GPU buffers.
///
...
pub fn gpu_cross_bf16(
    ...
    let n_u32 = n as u32;
    // SAFETY:
    // - `f` is compiled from `CROSS_BF16_PTX`; the entry signature is
    //   `(a_ptr, b_ptr, out_ptr, stride_axis, n)`.
    // - `a.len() == b.len() == n`, `out` is freshly allocated for `n`
    //   u16 bf16 bit-patterns, and `n > 0` here.
...
/// Elementwise scalar clamp on bf16 GPU buffers. Computed in f32 opmath and
/// rounded back to bf16 with round-to-nearest-even.
pub fn gpu_clamp_bf16(
    a: &cudarc::driver::CudaSlice<u16>,
    min_val: f32,
    max_val: f32,
    device: &GpuDevice,
...
/// Backward for scalar bf16 clamp. Grad/input are bf16 buffers; comparisons
/// use f32 opmath and output is rounded back to bf16.
pub fn gpu_clamp_backward_bf16(
    grad: &cudarc::driver::CudaSlice<u16>,
    input: &cudarc::driver::CudaSlice<u16>,
    min_val: f32,
    max_val: f32,
...
fn pow_bf16_ptx() -> String {
    crate::kernels::POW_PTX
        .replace("pow_kernel", "pow_bf16_kernel")
        .replace(
            ".reg .u64 %a, %out, %off;",
            ".reg .u64 %a, %out, %off;\n    .reg .b16 %a_b16, %zero16;\n    .reg .b32 %a_u32, %round, %lsb;",
        )
...
fn launch_pow_scalar(
    ...
    let ptx: &'static str = CACHE.get_or_init(pow_bf16_ptx).as_str();
    let ctx = device.context();
    let stream = device.stream();
    let f = get_or_compile(ctx, ptx, "pow_bf16_kernel", device.ordinal() as u32).map_err(|e| {
        GpuError::PtxCompileFailed {
            kernel: "pow_bf16_kernel",
            source: e,
        }
    })?;
    let mut out = stream.alloc_zeros::<u16>(n)?;
    ...
    let n_u32 = n as u32;
    // SAFETY:
    // - `f` resolves to `pow_bf16_kernel`, generated from the audited f32
    //   scalar-exponent `pow_kernel` with only entry-name, bf16 load/store,
    //   and 2-byte stride substitutions. ABI remains `(a_ptr, out_ptr,
    //   exponent, n)`, matching the four arguments pushed below.
    // - `a` is the caller's bf16 input buffer of length `n`; the PTX bound
...
const SIN_BF16_PTX: &str = "\
.version 7.0
.target sm_52
.address_size 64

.visible .entry sin_bf16_kernel(
    .param .u64 a_ptr,
    .param .u64 out_ptr,
    .param .u32 n
) {
...
const COS_BF16_PTX: &str = "\
.version 7.0
.target sm_52
.address_size 64

.visible .entry cos_bf16_kernel(
    .param .u64 a_ptr,
    .param .u64 out_ptr,
    .param .u32 n
) {
...
/// Elementwise `out = sin(a)` on bf16 GPU buffers with f32 internal math.
pub fn gpu_sin_bf16(
    a: &cudarc::driver::CudaSlice<u16>,
    device: &GpuDevice,
) -> GpuResult<cudarc::driver::CudaSlice<u16>> {
    launch_unary(a, device, SIN_BF16_PTX, "sin_bf16_kernel")
}

/// Elementwise `out = cos(a)` on bf16 GPU buffers with f32 internal math.
pub fn gpu_cos_bf16(
    a: &cudarc::driver::CudaSlice<u16>,
    device: &GpuDevice,
) -> GpuResult<cudarc::driver::CudaSlice<u16>> {
    launch_unary(a, device, COS_BF16_PTX, "cos_bf16_kernel")
}
...
/// Elementwise `out = a.pow(exponent)` on bf16 GPU buffers. The exponent is
/// already rounded to bf16 by the core dispatcher, matching PyTorch CUDA's
...
const EXTREME_AXIS_BF16_PTX: &str = "\
.version 7.0
.target sm_52
.address_size 64

.visible .entry extreme_axis_bf16_kernel(
    .param .u64 input_ptr,
    ...
    .reg .u32 %oi, %ii, %k, %base, %idx, %bits, %round, %lsb;
...
.visible .entry extreme_axis_backward_bf16_kernel(
    .param .u64 input_ptr,
...
fn validate_axis_extreme_dims_bf16(
    op: &'static str,
    input_len: usize,
    result_len: usize,
    grad_output_len: Option<usize>,
...
/// Axis `amin`/`amax`: bf16 [outer, axis, inner] -> bf16 [outer, inner].
/// Comparisons are performed in f32 registers with PyTorch NaN propagation.
pub fn gpu_extreme_axis_bf16(
    a: &cudarc::driver::CudaSlice<u16>,
    outer: usize,
    axis_size: usize,
    inner: usize,
    ...
    let (_, total) = validate_axis_extreme_dims_bf16(
        "extreme_axis_bf16",
        a.len(),
        expected_result_len,
        None,
    ...
    let f = get_or_compile(
        ctx,
        EXTREME_AXIS_BF16_PTX,
        "extreme_axis_bf16_kernel",
        device.ordinal() as u32,
    )
    .map_err(|e| GpuError::PtxCompileFailed {
        kernel: "extreme_axis_bf16_kernel",
        source: e,
    })?;
    let mut out = stream.alloc_zeros::<u16>(total)?;
    let cfg = launch_1d(total);
    ...
    // SAFETY:
    // - `f` is `extreme_axis_bf16_kernel` with args
    //   (input, output, outer, axis_size, inner, total_output, is_max).
    // - `validate_axis_extreme_dims_bf16` proves the input is exactly
    //   `[outer, axis_size, inner]`, `axis_size > 0`, and all indices fit
    //   the kernel's u32 arithmetic.
    // - `out` is a fresh `[outer, inner]` bf16 allocation; each thread writes
    //   one output element.
    unsafe {
        stream
            .launch_builder(&f)
...
/// Backward for bf16 axis `amin`/`amax`.
///
...
pub fn gpu_extreme_axis_backward_bf16(
    input: &cudarc::driver::CudaSlice<u16>,
    ...
    let (total_input, _) = validate_axis_extreme_dims_bf16(
        "extreme_axis_backward_bf16",
        input.len(),
        result.len(),
    ...
    let f = get_or_compile(
        ctx,
        EXTREME_AXIS_BF16_PTX,
        "extreme_axis_backward_bf16_kernel",
        device.ordinal() as u32,
    )
    .map_err(|e| GpuError::PtxCompileFailed {
        kernel: "extreme_axis_backward_bf16_kernel",
        source: e,
    })?;
    ...
    // SAFETY:
    // - `f` is `extreme_axis_backward_bf16_kernel` with args
