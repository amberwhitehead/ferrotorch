...
//! | REQ-8 (`_into` zero-alloc variants) | NOT-STARTED | `pub fn gpu_matmul_f32_into / gpu_bmm_f32_into in blas.rs` exist as vocabulary but workspace-wide audit (`ferrotorch-gpu/tests/divergence_blas_req8_into_consumers.rs`) finds ZERO non-test consumers — both symbols are also in `ferrotorch-gpu/tests/conformance/_surface_exclusions.toml` with `reason = "deferred"`. Blocked on #1360 (wire the zero-host-bounce dispatch path) |
