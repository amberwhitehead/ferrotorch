#!/usr/bin/env python3
"""
translate-discipline hook (ferrotorch ← PyTorch translation gate).

Enforces "look at the PyTorch source, look at the design doc, look at
goal.md; then translate to Rust" as a deterministic per-edit gate.

Two invocations in .claude/settings.json:

  - PostToolUse on Read    -> records the Read in session state
  - PreToolUse  on Write|Edit -> gates writes to ferrotorch-*/src/**/*.rs

State is persisted at .crosslink/.translate-reads.json (per-worktree).

The three required source classes for any .rs edit under ferrotorch-*/src/:
  1. goal.md                          (always; global discipline file)
  2. /home/doll/pytorch/<route.upstream>  (per route; ≥1 path Read)
...
If a design doc is missing, the hook BLOCKS with instructions to dispatch
acto-doc-author. Both blocks include the priority statement about
...
# ─── path predicate: is this a gated .rs file? ──────────────────────────

def is_gated_rs_path(rel_path):
    """Return True iff this is a ferrotorch-*/src/**/*.rs file we gate.

    ferrotorch-nn-derive is a proc-macro crate with no PyTorch counterpart
    and is explicitly excluded from the gate.

    The meta-crate `ferrotorch/src/**/*.rs` IS gated — it should be
    re-exports only, but if it accumulates logic the gate catches it.
    """
...
# ─── priority footer (printed at the end of every block message) ────────
...
def main():
    ...
    # ── PreToolUse on Write|Edit: gate writes to ferrotorch-*/src/**/*.rs ──
    ...
    if not matched:
        print(
            ...
            f"\n"
            f"Every .rs file under ferrotorch-*/src/**/*.rs MUST have a route\n"
            f"declaring its upstream PyTorch source and design doc. Without a\n"
            ...
            f"  [[route]]\n"
            f"  crate_pattern = \"{rel}\"\n"
            f"  upstream = [\"/home/doll/pytorch/<area>/<file>.cpp\"]\n"
            f"  design = \".design/<area>/<doc>.md\"\n"
            f"  parity_ops = []   # or [\"add\", \"sub\", ...] if this file owns parity ops\n"
            f"\n"
    ...
    for route in matched:
        ...
        if design_path:
            ...
            if not design_exists:
                ...
                print(
                    ...
                    f"  Read it (and the upstream PyTorch sources), then\n"
                    f"  retry the edit.\n"
                    f"{PRIORITY_FOOTER}"
                )
                sys.exit(2)
        ...
        if missing:
            print(
                ...
                f"\n"
                f"This file is a Rust translation of upstream PyTorch. Before\n"
                f"any edit, you MUST Read each of the three required source\n"
                f"classes:\n"
                f"  1. goal.md                  (always; global discipline)\n"
                f"  2. /home/doll/pytorch/      (the upstream C++/Python source)\n"
