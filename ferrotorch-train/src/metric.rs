...
//! | REQ-3 | SHIPPED | impl: `pub struct AccuracyMetric` in `metric.rs`; consumer: `Learner::with_accuracy_metric` in `ferrotorch-train/src/learner.rs` accepts the metric plus a `ClassificationAdapter<T>` closure that maps `(pred, target)` tensors to `(correct, total)` and drives `metric.update(&pair)` inside the per-batch `fit` loop; non-test caller: `ferrotorch-train/examples/multi_epoch_train_dump.rs::run_learner_smoke` attaches an argmax-of-pred-vs-target adapter and reads the value via `learner.metric_snapshot()`. Closes #1494. |
//! | REQ-4 | SHIPPED | impl: `pub struct TopKAccuracy` in `metric.rs`; consumer: `Learner::with_topk_accuracy_metric` in `ferrotorch-train/src/learner.rs` wires the same adapter pattern as REQ-3; non-test caller: `ferrotorch-train/examples/multi_epoch_train_dump.rs::run_learner_smoke` attaches `TopKAccuracy::new(3)` with a top-3 partial-sort adapter. Closes #1495. |
//! | REQ-5 | SHIPPED | impl: `pub struct RunningAverage` in `metric.rs`; consumer: `Learner::with_running_average_metric` in `ferrotorch-train/src/learner.rs` registers the metric on a dedicated `running_average_metrics` slot independent of `train_metrics` so the sliding window survives epoch-boundary resets; non-test caller: `ferrotorch-train/examples/multi_epoch_train_dump.rs::run_learner_smoke` attaches `RunningAverage::new(8)` and reads the value via `metric_snapshot()`. Closes #1496. |
